...
typedef struct
{
    unsigned long frame_generate_time[TEST_DISPLAY_BUFFER_NUM];
    unsigned long buf_commit_time[TEST_DISPLAY_BUFFER_NUM];
    unsigned long buf_rel_time[TEST_DISPLAY_BUFFER_NUM];
    int bprint;
} test_util_diag_t;
...
typedef struct
{
    int buffer_size[2];     ///< Buffer size for output of ISP [width, height]
    int stride;
