    ...
    mCamera(pCamera)
{
    ...
    // Add more buffer since we are doing RGB conversion
    pCamera->setMaxFramesInFlight(8);
}
...
Return<void> StreamHandler::deliverFrame(const BufferDesc& buffer) {
    //ALOGI("Received a frame from the camera (%p)", buffer.memHandle.getNativeHandle());
