...
#define AIS_LOG_LVL_ERR       3                    //error
 ...
 */
#define AIS_LOG_ENTER(id, level, fmt, ...) ais_log((id), (level),  "%s E %s " fmt "\n", AIS_LOG_LVL_STR[AIS_LOG_LVL_CORR(level)], __FUNCTION__, ##__VA_ARGS__)
#define AIS_LOG_EXIT(id, level, fmt, ...) ais_log((id), (level),  "%s X %s " fmt "\n", AIS_LOG_LVL_STR[AIS_LOG_LVL_CORR(level)], __FUNCTION__, ##__VA_ARGS__)


/**
 * a simulation of assert in libc
 */
#define AIS_LOG_ASSERT(x) ((void)((x) || ais_log(AIS_MOD_ID_RSVR, AIS_LOG_LVL_ALWAYS, "%s:%s:%d ASSERT %s\n\n", __FILE__, __FUNCTION__, __LINE__, #x)))
 ...
 * 2, add MODULE_xx=y in crosponding ais_log.conf
...
#define CAM_MSG_ERROR_LEVEL     AIS_LOG_LVL_ERR    // orginal value 5
...
#define CAM_MSG(_level_, _fmt_, ...)                                           \
    ...
    {                                                                          \
        ...
        ais_log(AIS_MOD_ID_RSVR, _lvl_,  "%s:%d %s " _fmt_ "\n",               \
