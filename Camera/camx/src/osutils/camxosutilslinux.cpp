...
#if defined (_LINUX) || defined(__QNXNTO__)  // This file for Linux build only
...
//For unit testing, need to track memory so use calloc
    ...
    SIZE_T sizeDst)
{
    ...
    while ((nLen < sizeDst) && (0 != pDst[nLen])) {
       ++nLen;
    }

    return nLen + StrLCpy(pDst+nLen, pSrc, sizeDst-nLen);
#else
    ...
    FormatParamInfo*   pFormatParamInfo)
{
    ...
    if (FALSE == pFormatParamInfo->isImplDefined)
    {
        ...
        else if (GrallocUsagePrivateHEIF == (pFormatParamInfo->grallocUsage & GrallocUsagePrivateHEIF))
        {
