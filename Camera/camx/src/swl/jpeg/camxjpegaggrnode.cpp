    ...
    ExecuteProcessRequestData* pExecuteProcessRequestData)
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupJPEG, "NULL param pEnabledPortsInfo %p", pExecuteProcessRequestData->pEnabledPortsInfo);
        thumbnailNeeded = FALSE;
    }
        ...
        if (CamxResultSuccess == result)
        {
            ...
            else if (TRUE == m_rotationHandledFCV)
            {
                ...
                if (CamxResultSuccess == result)
                {
                    ...
                    length = CAMX_ARRAY_SIZE(PropertiesFCVRotation);

                    GetDataList(PropertiesFCVRotation, pFCVRotationData, propertyDataFCVRotationOffset, length);
    ...
    PerRequestActivePorts*    pEnabledPorts)
{
    ...
    if (TRUE == bThumbnailNeeded && (TRUE == IsTagPresentInPublishList(PropertyIDJPEGEncodeOutInfoThumbnail)))
    {
        pNodeRequestData->dependencyInfo[0].dependencyFlags.hasPropertyDependency = TRUE;
