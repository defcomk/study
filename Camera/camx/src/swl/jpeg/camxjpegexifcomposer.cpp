...
CamxResult JPEGEXIFComposer::StartComposition()
{
    ...
    if ((NULL == m_pInputMainBuf) || (NULL == m_pOutputBuf) || (NULL == m_pAheadBuf) || (NULL == m_pEXIFParams))
    {
        CAMX_LOG_ERROR(CamxLogGroupJPEG, "NULL params m_pInputMainBuf %p m_pOutputBuf%p m_pAheadBuf %p m_pEXIFParams%p",
            m_pInputMainBuf, m_pOutputBuf, m_pAheadBuf, m_pEXIFParams);
    ...
    if (CamxResultSuccess == result)
    {
        ...
        CAMX_LOG_INFO(CamxLogGroupJPEG, "EXIF header done, encoded size %d offset %x", encoderBufferFilled,
                             m_currentOffset);
    ...
    if (CamxResultSuccess == result)
    {
        CAMX_LOG_WARN(CamxLogGroupJPEG, "maxJPEGSize not set using bufsize %d", m_outputBufSize);
...
CamxResult JPEGEXIFComposer::FlushFileHeader()
{
    ...
    if ((m_currentOffset + m_statsDebugDataSize + m_pEXIFParams->GetEncodeOutParams()->bufferFilledSize) <
        static_cast<UINT32>(m_pOutputBuf->GetPlaneSize(0))) {

        if (NULL != m_pStatsDebugDataAddr) {
            CAMX_LOG_INFO(CamxLogGroupJPEG, "DebugDataAll: EXIF writing total: %p, size: %u, 3A enable: %u, Tuning enable: %u "
        ...
        {
            CAMX_LOG_INFO(CamxLogGroupJPEG, "DebugDataAll: Not writing 3A debug data or Tuning metadata ! 3A enable: %u"
                "Tuning enable: %u", pStaticSettings->enable3ADebugData, pStaticSettings->enableTuningMetadata);
        }
    }
    else
    {
        CAMX_LOG_ERROR(CamxLogGroupJPEG, "Skip Large Debug Data");
    }
