...
VOID JPEGEXIFParams::GetEXIFOrientation()
{
    ...
    else
    {
        ...
        CAMX_LOG_ERROR(CamxLogGroupJPEG, "Exif orientation is not received");
    }
...
VOID JPEGEXIFParams::GetEXIFFocalLength()
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupJPEG, "PropertyIDSensorProperties not published");
    }
}
...
VOID JPEGEXIFParams::GetEXIFExposure()
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupJPEG, "AEC Exposure Info:NULL");
    }

    if (NULL != pData[3])
    {
        pASDOutput = reinterpret_cast<ASDOutput*>(pData[3]);
        CAMX_LOG_VERBOSE(CamxLogGroupJPEG, "Detected (PT:%d, BL:%d LS:%d HDR:%d)",
                         pASDOutput->detected[ASDPortrait],
                         pASDOutput->detected[ASDBacklight],
                         pASDOutput->detected[ASDLandscape],
                         pASDOutput->detected[ASDHDR]);
    }
    else
    {
        CAMX_LOG_ERROR(CamxLogGroupJPEG, "ASD Output:NULL");
    }
...
CamxResult JPEGEXIFParams::GetEXIFData()
{
    ...
    if ( (FALSE == m_bIsGpuNodePresent) ||
            (FALSE == HwEnvironment::GetInstance()->GetSettingsManager()->GetStaticSettings()->overrideGPURotationUsecase))
    {
        GetEXIFOrientation();
    }
    GetEXIFCustomQuantizationtables();
    GetJPEGQualityAndUpdateQuantizationTable();
    if ((0 != m_imageParamsThumb.imgFormat.width) && (0 != m_imageParamsThumb.imgFormat.height))
    {
