    ...
    RegisterSetting* pSettings)
{
    ...
    UINT               i                = 0;
    UINT               address_changed  = 0;

    while ( i < settingsCount )
    {
        if (TRUE == pSettings[i].slaveAddrExists)
        {
            if (0 == prevSlaveAddr || prevSlaveAddr != pSettings[i].slaveAddr)
            {
    ...
    while (i < settingsCount)
    {
        ...
        switch (pSettings[i].operation)
        {
            ...
            case OperationType::READ:
                ...
                pReadCmd->header.count    = static_cast<UINT16>(pSettings[i].registerData[0]);
            ...
            case OperationType::WRITE_SEQUENTIAL:
                {
                    ...
                    pBurstCmd->header.count    = static_cast<UINT16>(pSettings[i].registerDataCount);
    ...
    BOOL                                isD65)
{
    ...
    MatrixMultiplication3x3(sRGB2XYZ, colorCorrectionMatrix, outputMatrix);
    FloatToRational3x3(outputMatrix, forwardMatrix);

    if (TRUE == isD65)
    {
        MatrixMultiplication3x3(XYZ2RGB, colorCorrectionMatrix, temporaryMatrix1);
    }
    else
    {
        MatrixMultiplication3x3(D65_to_ref_A, XYZ2RGB, temporaryMatrix);
        MatrixMultiplication3x3(temporaryMatrix, colorCorrectionMatrix, temporaryMatrix1);
    }
    MatrixMultiplication3x3(temporaryMatrix1, MWBMatrix, temporaryMatrix2);

    pPtr1 = reinterpret_cast<FLOAT*>(temporaryMatrix2);
    pPtr2 = reinterpret_cast<FLOAT*>(outputMatrix);
    MatrixInverse3x3(pPtr1, pPtr2);
