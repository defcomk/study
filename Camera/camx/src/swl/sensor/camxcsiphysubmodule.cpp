    ...
    const CHAR*       pDeviceName)
{
    ...
    CAMX_ASSERT(NULL != ppCSIPHYSubModule);

    pCSIPHY = CAMX_NEW CSIPHYSubmodule(pHwContext);
    if (NULL == pCSIPHY)
    {
        CAMX_LOG_ERROR(CamxLogGroupSensor, "CSIPHY subomdule creation failed - out of memory");
    ...
    else
    {
        ...
        {
            CAMX_LOG_ERROR(CamxLogGroupSensor, "CSIPHY sub-module initialization failed result: %d", result);
            pCSIPHY->Destroy();
            pCSIPHY = NULL;
        }
    }

    CAMX_ASSERT(NULL != pCSIPHY);
