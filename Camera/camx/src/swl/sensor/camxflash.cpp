    ...
    FlashCreateData* pCreateData)
{
    ...
    if (NULL != pHwContext)
    {
        ...
        if (NULL == pFlashData)
        {
            ...
            CAMX_LOG_WARN(CamxLogGroupSensor, "FlashData is NULL, flash is not support on cameraId %d", pCreateData->cameraId);
        }
        else
        {
    ...
    if (TRUE == bNormalFlashControl)
    {
        ...
        else if (FlashInfoTypeCalibration == flashType)
        {
                ...
                LEDCurrents[LEDSetting2] != 0)
            {
                m_highCurrent[0] = LEDCurrents[LEDSetting1];
                m_highCurrent[1] = LEDCurrents[LEDSetting2];
                CAMX_LOG_VERBOSE(CamxLogGroupStats, "m_highCurrent[0] %d m_highCurrent[1] %d",
                    m_highCurrent[0],
                    m_highCurrent[1]);
            }
            else
    ...
    BOOL* pShouldSendNop)
{
    ...
    else if (0 == m_flashOffDelayCounter)
    {
    ...
    if (NULL != pData[0])
    {
        ...
        if (m_preFlashState != preFlashState)
        {
            ...
            else
            {
                ...
                CAMX_LOG_VERBOSE(CamxLogGroupSensor, "PreFlashState Other:%d -> Need to send NoOp");
                *pShouldSendNop = TRUE;
            }
