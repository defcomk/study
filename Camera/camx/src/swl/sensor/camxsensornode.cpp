    ...
    SubDevice type)
{
    ...
    if ((MaxNumImageSensors > cameraId) && (MaxSubDevices > type))
    {
        ...
        {
            CAMX_LOG_WARN(CamxLogGroupSensor, "CameraID=%u Subdevice=%u refCount already 0!", cameraId, type);
        }
...
VOID SensorNode::PopulatePDAFInformation()
{
    ...
    if (NULL != pPDAFConfigData)
    {
        CAMX_LOG_INFO(CamxLogGroupSensor,
                      "pdtype %d, PDAFname: %s",
                      pPDAFConfigData->PDInfo.PDType,
                      pPDAFConfigData->PDInfo.PDAFName);
        if ((PDAFType::PDType2   == pPDAFConfigData->PDInfo.PDType) ||
            (PDAFType::PDType2PD == pPDAFConfigData->PDInfo.PDType) ||
            (PDAFType::PDType1 == pPDAFConfigData->PDInfo.PDType))
        {
            m_sensorPDAFInfo.PDAFBufferFormat = static_cast<PDBufferFormat>(
                pPDAFConfigData->PDBufferBlockPatternInfo[resIdx].PDBufferFormat);

            m_sensorPDAFInfo.PDAFNativeBufferFormat = static_cast<PDBufferFormat>(
                pPDAFConfigData->PDSensorNativePatternInfo[resIdx].PDNativeBufferFormat);
        }

        m_sensorPDAFInfo.PDAFSensorType = pPDAFConfigData->PDInfo.PDType;

        if (TRUE == pPDAFConfigData->PDSensorNativePatternInfo[resIdx].PDBlockPatternExists)
        {
    ...
    UINT64 requestId)
{
    ...
    ColorFilterArrangement colorFilterArrangement =
        GetSensorDataObject()->GetColorFilterPattern(m_currentResolutionIndex);
    ...
    CAMX_LOG_VERBOSE(CamxLogGroupSensor,
                     "Publishing metadata: ReqId: %llu resID %d rollingShutterSkew: %llu , FilterArrangement: %d, "
    ...
    if (NULL == m_pSensorModuleData->GetActuatorDataObject())
    {
        ...
        UINT lens_state                = LensStateStationary;
        const void * pLensStateData[1] = { &lens_state};
    ...
    NodeProcessRequestData* pNodeProcessRequestData)
{
    ...
    {
        CAMX_LOG_VERBOSE(CamxLogGroupMeta, "pipeline cannot publish property: %08x ", PropertyIDAECFrameControl);
    }
    ...
    {
        CAMX_LOG_VERBOSE(CamxLogGroupMeta, "pipeline cannot publish property: %08x ", PropertyIDAFFrameControl);
        if (NULL != m_pActuator)
        {
            // Since actuator is a realtime device need to send NOP packet AF node is not enabled
    ...
    {
        CAMX_LOG_VERBOSE(CamxLogGroupMeta, "pipeline cannot publish property: %08x ", PropertyIDAWBFrameControl);
    }
    ...
    {
        CAMX_LOG_VERBOSE(CamxLogGroupMeta, "pipeline cannot publish property: %08x ", regControlMetaTag);
    }
        ...
        (TRUE != m_isMasterCamera))
    {
        ...
        {
            CAMX_LOG_VERBOSE(CamxLogGroupMeta, "pipeline cannot publish property: %08x ", PropertyIDSensorMetaData);
        }
        ...
        {
            CAMX_LOG_VERBOSE(CamxLogGroupMeta, "pipeline cannot publish property: %08x ", PropertyIDSensorResolutionInfo);
        }
    ...
    ExecuteProcessRequestData* pExecuteProcessRequestData)
{
    ...
    if ((NULL != pStaticSettings) && (NULL != pExecuteProcessRequestData) &&
        ...
        (NULL != pExecuteProcessRequestData->pTuningModeData))
    {
        ...
        else if (CamxResultSuccess == result)
        {
            if (TRUE == IsIHDRSupported())
            {
                ...
                // query if 3-expo-hdr
                VendorTagManager::QueryVendorTagLocation("com.qti.ihdr_control", "state_ihdr_snapshot", &tag[0]);
                tag[0] |= InputMetadataSectionMask;
                GetDataList(tag, reinterpret_cast<VOID **>(pData), configDataOffset, length);
                if (NULL != pData[0])
                {
                    m_stateIHDRSnapshot = static_cast<IHDRSnapshotState>(*pData[0]);
                    if (IHDR_NONE != m_stateIHDRSnapshot)
                    {
                        CAMX_LOG_INFO(CamxLogGroupSensor, "m_stateIHDRSnapshot %d, requestId %d",
                            m_stateIHDRSnapshot, requestId);
                    }
                }

                if (IsIHDRSnapshotEnabled())
                {
            ...
            if (FALSE == IsStatsNodeEnabled())
            {
                ...
                else
                {
                    ...
                    {
                        updateParam.currentFrameLengthLines = GetSensorDataObject()->GetFrameLengthLines(
                                                                                        m_currentResolutionIndex);
                    }
                }
            ...
            else
            {
                ...
                if (NULL != pAECOutput)
                {
                    ...
                    UpdateSensorExposure(applyShortExposure, applyMiddleExposure, &updateParam, pAECOutput);
                    CAMX_LOG_INFO(CamxLogGroupSensor,
                ...
                if (TRUE == GetSensorDataObject()->Is3ExposureHDRMode(m_currentResolutionIndex))
                {
                    ...
                    {
                        CAMX_LOG_ERROR(CamxLogGroupSensor, "Adjust ISP gain fail when EV compensation in 3-exposure HDR mode!");
                    }
                }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// SensorNode::IsIHDRSupported
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
BOOL SensorNode::IsIHDRSupported()
{
    CamxResult            result     = CamxResultSuccess;
    HwCameraInfo          cameraInfo;
    ...
    if (CamxResultSuccess == result)
    {
        return (cameraInfo.pSensorCaps->isIHDRSupported);
    }
    ...
    NodeProcessRequestData* pNodeProcessRequestData)
{
    ...
    if ((CamxResultSuccess == result) && (NULL != pData[0]) && (NULL != pData[1]))
    {
        ...
        if ((NULL != pMasterMetaData) && (NULL != pMasterResoultionInfo))
        {
            ...
            FLOAT slaveFps = (static_cast<FLOAT>(GetSensorDataObject()->GetFrameLengthLines(m_currentResolutionIndex)) /
                             static_cast<FLOAT>(pSensorParam->currentFrameLengthLines)) *
            ...
            CAMX_LOG_VERBOSE(CamxLogGroupSensor,
                             "requestID:%llu PeerRequestID:%llu masterFPS: %f, slaveFPS: %f slave FLL:%u,"
                             " slaveMaxFPS %f, adjusted slave FLL = %u",
                             requestId,
                             pMultiRequestData->currReq.requestID[m_peerPipelineId],
                             masterFps, slaveFps,
                             GetSensorDataObject()->GetFrameLengthLines(m_currentResolutionIndex),
                             GetSensorDataObject()->GetMaxFPS(m_currentResolutionIndex),
    ...
    FinalizeInitializationData* pFinalizeInitializationData)
{
    ...
    CAMX_LOG_CONFIG(CamxLogGroupSensor, "CameraID:%d Sensor Sync Mode:%d" , m_cameraId, m_currentSyncMode);
    ...
    BOOL isStatsNodeEnabled)
{
    ...
    {
        applyShortExposure = (GetSensorDataObject()->IsHDRMode(m_currentResolutionIndex)) || IsIHDRSnapshotEnabled();
        applyMiddleExposure = (GetSensorDataObject()->Is3ExposureHDRMode(m_currentResolutionIndex)) || IsIHDRSnapshotEnabled();

        SensorParam updateParam = { 0 };
        if (TRUE == isStatsNodeEnabled)
        {
            AECFrameControl* pControl = reinterpret_cast<AECFrameControl*>(pUsecaseData[0]);
            UpdateSensorExposure(applyShortExposure, applyMiddleExposure, &updateParam, pControl);
        }
        else
        {
        ...
        CAMX_LOG_VERBOSE(CamxLogGroupSensor,
            "Sensor: Update Startup Exposure: Gain = %f ExpTime = %ld, Middle Gain = %f MIddle ExpTime = %ld, "
            "Short Gain = %f Short ExpTime = %ld FLL = %d",
            updateParam.currentGain,
            updateParam.currentExposure,
            updateParam.currentMiddleGain,
            updateParam.currentMiddleExposure,
    ...
    if ((CamxResultSuccess == result) && (NULL != m_pConfigCmdBufferManager) && (NULL != m_pConfigPacketManager))
    {
        ...
        else
        {
            ...
            if (NULL != pCmd)
            {
                ...
                {
                    result = pSensorData->CreateI2CCmd(cmdType, pCmd, NULL, currentResolutionIndex);
                }
                if (CamxResultSuccess == result)
                {
...
CamxResult SensorNode::LoadSensorConfigCmds()
{
    ...
    if (SensorConfigurationStatus::SensorSubModuleCreateComplete == m_sensorConfigStatus)
    {
        ...
        UINT resCmdSize   = pSensorData->GetI2CCmdSize(I2CRegSettingType::Res, NULL, m_currentResolutionIndex);
        UINT startCmdSize = pSensorData->GetI2CCmdSize(I2CRegSettingType::Start, NULL, 0);
        UINT stopCmdSize  = pSensorData->GetI2CCmdSize(I2CRegSettingType::Stop, NULL, 0);
        ...
        {
            syncCmdSize    = pSensorData->GetI2CCmdSize(I2CRegSettingType::Master, NULL, 0);
            syncI2CCmdType = I2CRegSettingType::Master;
        }
        else if (SlaveMode == m_currentSyncMode)
        {
            syncCmdSize    = pSensorData->GetI2CCmdSize(I2CRegSettingType::Slave, NULL, 0);
        ...
        if (CamxResultSuccess == result)
        {
            ...
            if (NoSync != m_currentSyncMode)
            {
                ...
                else
                {
                    result = CreateAndSubmitCommand(syncCmdSize, syncI2CCmdType,
                             CSLPacketOpcodesSensorConfig, 0);
            ...
            if (CamxResultSuccess == result)
            {
                result = CreateAndSubmitCommand(startCmdSize, I2CRegSettingType::Start,
                         CSLPacketOpcodesSensorStreamOn, 0);
                ...
                if (CamxResultSuccess == result)
                {
                    ...
                    result = CreateAndSubmitCommand(stopCmdSize, I2CRegSettingType::Stop,
                             CSLPacketOpcodesSensorStreamOff, 0);
...
CamxResult SensorNode::CreatePerFrameUpdateResources()
{
    ...
    if ((NULL == m_pExposureInfo) || (NULL == m_pRegSettings) ||
        (NULL == m_pExposureRegAddressInfo) || (NULL == m_pPDAFSettings) || (NULL == m_pWBGainSettings))
    {
        CAMX_LOG_ERROR(CamxLogGroupSensor,
    ...
    NodeProcessRequestData* pNodeProcessRequestData)
{
    ...
    BOOL    applyShortExposure  = (GetSensorDataObject()->IsHDRMode(m_currentResolutionIndex)) || IsIHDRSnapshotEnabled();
    BOOL    applyMiddleExposure = (GetSensorDataObject()->Is3ExposureHDRMode(m_currentResolutionIndex)) ||
                                    IsIHDRSnapshotEnabled();
/*
    //  Debug for I2C register
    RegSettingsInfo* pRegSettingsInfo = reinterpret_cast<RegSettingsInfo*>(m_pRegSettings);
    if ((NULL != pRegSettingsInfo) && (TRUE == IsIHDRSnapshotEnabled()))
    {
        for (UINT regCount = 0; regCount < pRegSettingsInfo->regSettingCount; regCount++)
        {
            CAMX_LOG_INFO(CamxLogGroupSensor, "[IHDR] Expo: regcnt=%u, addr=%04x, type=%u, data=%02x, data=%u, delay=%u",
                                    regCount,
    ...
    UINT64  requestId)
{
    ...
    if ((pStaticSettings->enableSensorHWSync) &&
        (0 == m_sensorSyncTag) &&
        (TRUE == m_isMultiCameraUsecase))
    {
    ...
    if ((pStaticSettings->enableSensorHWSync) &&
        ...
        (TRUE == m_isMultiCameraUsecase))
    {
        static const UINT SensorSyncProperties[] =
        {
            m_sensorSyncTag | StaticMetadataSectionMask
        };
        VOID*  pSensorSyncData[1]      = { 0 };
        UINT64 sensorSyncDataOffset[1] = { 0 };
        UINT   length;

        length = CAMX_ARRAY_SIZE(SensorSyncProperties);
        result = GetDataList(SensorSyncProperties, pSensorSyncData, sensorSyncDataOffset, length);

        SensorSyncModeMetadata* pSensorSyncMeta = static_cast<SensorSyncModeMetadata*>(pSensorSyncData[0]);
        if ((NULL != pSensorSyncMeta) && (TRUE == pSensorSyncMeta->isValid))
        {
            syncMode = pSensorSyncMeta->sensorSyncMode;
        }
        else
        {
            CAMX_LOG_ERROR(CamxLogGroupSensor, "Get sensor sync meta failed %p", pSensorSyncMeta);
        }
    }
    ...
    if (syncMode != m_currentSyncMode)
    {
        ...
        if (syncMode == MasterMode)
        {
            ...
            dualCmdSize = GetSensorDataObject()->GetI2CCmdSize(I2CRegSettingType::Master, NULL, 0);
        }
        else if (syncMode == SlaveMode)
        {
            i2cSettingType = I2CRegSettingType::Slave;
            dualCmdSize = GetSensorDataObject()->GetI2CCmdSize(I2CRegSettingType::Slave, NULL, 0);
        }
        ...
        if (dualCmdSize != 0)
        {
            ...
            if (pDualUpdate != NULL)
            {
                result = GetSensorDataObject()->CreateI2CCmd(i2cSettingType, pDualUpdate, NULL, 0);
                if (CamxResultSuccess == result)
                {
    ...
    if ((TRUE == IsIHDRSupported()) && ((IHDR_START == m_stateIHDRSnapshot) || (IHDR_STOP == m_stateIHDRSnapshot)))
    {
        UINT ihdrCmdSize = 0;
        I2CRegSettingType i2cSettingType = I2CRegSettingType::IHDR_OFF;

        if (IHDR_START == m_stateIHDRSnapshot)
        {
            i2cSettingType = I2CRegSettingType::IHDR_ON;
        }
        else
        {
            i2cSettingType = I2CRegSettingType::IHDR_OFF;
        }

        ihdrCmdSize = GetSensorDataObject()->GetI2CCmdSize(i2cSettingType, NULL, 0);
        if (0 != ihdrCmdSize)
        {
            CAMX_LOG_INFO(CamxLogGroupSensor, "IHDR requestId: %llu, cameraId %d, ihdrCmdSize:%d, i2cSettingType %d",
                                                requestId, m_cameraId, ihdrCmdSize, i2cSettingType);
            VOID* pIHDRUpdate = m_pUpdateCmds->BeginCommands(ihdrCmdSize / sizeof(UINT32));
            if (pIHDRUpdate != NULL)
            {
                result = GetSensorDataObject()->CreateI2CCmd(i2cSettingType, pIHDRUpdate, NULL, 0);
                if (CamxResultSuccess == result)
                {
                    result = m_pUpdateCmds->CommitCommands();
                }
    ...
    if (IHDR_STOP == m_stateIHDRSnapshot)
    {
        m_stateIHDRSnapshot = IHDR_NONE;
    }
        ...
        if (0 != pdafUpdateCmdSize)
        {
            ...
            if (NULL != pCmdBegin)
            {
                ...
                result = GetSensorDataObject()->CreateI2CCmd(I2CRegSettingType::PDAF,
                                  pCmdPDAFUpdate, m_pPDAFSettings, 0);
        ...
        if (0 != awbGainUpdateCmdSize)
        {
            ...
            if (NULL != pCmdBegin)
            {
                ...
                result = GetSensorDataObject()->CreateI2CCmd(I2CRegSettingType::AWB,
                                  pCmdWBGainUpdate, m_pWBGainSettings, 0);
    ...
    if (CamxResultSuccess == result && (isNeedUpdateSyncMode || isNeedUpdateAEC || isNeedUpdatePDAF || isNeedUpdateWBGain))
    {
...
DOUBLE SensorNode::GetFpsDiv()
{
    ...
    BOOL isFastShutterEnabled                = FALSE;
    CamxResult result                        = CamxResultEFailed;
    ...
    result = IsFSModeEnabled(&isFastShutterEnabled);
    ...
    if (HALmaxFPS != 0)
    {
        if (ControlCaptureIntentStillCapture == captureIntent && TRUE == isFastShutterEnabled)
        {
            CAMX_LOG_VERBOSE(CamxLogGroupSensor,
...
CamxResult SensorNode::LoadSensorInitConfigCmd()
{
    ...
    if (SensorConfigurationStatus::SensorConfigurationStateUninitialized == m_sensorConfigStatus)
    {
        ...
        do
        {
            ...
            initializeCmdSize = GetSensorDataObject()->GetI2CCmdSize(I2CRegSettingType::Init, NULL, 0);
            ...
            result = CreateAndSubmitCommand(initializeCmdSize, I2CRegSettingType::Init,
                CSLPacketOpcodesSensorInitialConfig, 0);
    ...
    PDAFData* pPDAFData)
{
    ...
    if (PDAFType::PDType3 == PDType)
    {
        ...
        {
            CAMX_LOG_WARN(CamxLogGroupSensor, "Cannot get IFE CAMIF T3 Extracted pattern");
            result = CamxResultEFailed;
        }
    }
