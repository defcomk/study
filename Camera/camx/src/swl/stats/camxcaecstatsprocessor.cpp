    ...
    const StatsInitializeData* pInitializeData)
{
    ...
    if (FALSE == isEarlyPCREnabled && CamxResultSuccess == result)
    {
    ...
    const StatsInitializeData* pInitializeData)
{
    ...
    if (NULL != m_tuningData.pTuningSetManager)
    {
    ...
    {
        CAMX_LOG_WARN(CamxLogGroupAEC, "pTuningSetManager is NULL");
    }
}
    ...
    const StatsProcessRequestData* pStatsProcessRequestDataInfo)
{
    ...
    m_pStatsAECPropertyReadWrite->GetReadProperties(AECReadTypePropertyIDCount);
    ...
    AECEngineAlgorithmInput*       pInput)
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupAEC, "Invalid Argument pInput:%p pStatsProcessRequestDataInfo:%p");
        return CamxResultEInvalidPointer;
    }
    ...
    AECEngineAlgorithmInput*       pInput)
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        CAMX_LOG_VERBOSE(CamxLogGroupAEC, "Setting camera information cameraId:%d role:%d cameraType:%d",
            pInput->cameraInfo.cameraId, pInput->cameraInfo.algoRole, pInput->cameraInfo.cameraType);
    }
    ...
    AECEngineProcessStatsOutput*   pOutput)
{
    ...
    CamxResult result = StatsUtil::WriteVendorTag(m_pNode,
    ...
    if (CamxResultSuccess != result)
    {
        CAMX_LOG_ERROR(CamxLogGroupAEC, "cannot write to sensor register control vendor tag");
    }
...
CamxResult CAECStatsProcessor::PublishPropertyDebugData()
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupAEC,
            "cannot write to org.quic.camera.debugdata and DebugDataAll vendor tag");
    }
    return result;
}
    ...
    MetadataPool* pUsecasePool)
{
    ...
    CAMX_LOG_INFO(CamxLogGroupISP, "Writing to UsecasePool by AEC");
    return result;
}
    ...
    AECFrameControl*             pFrameControlOut)
{
    ...
    AECAlgoExposureData*   pExposureData        = NULL;
    FLOAT                  aecSensitivity[ExposureIndexCount];
    ...
    pFrameControlOut->isInsensorHDRSnapshot    = pEngineFrameControl->frameControl.isInsensorHDRSnapshot;
    ...
    for (UINT32 i = 0; i < ExposureIndexCount; i++)
    {
        ...
        aecSensitivity[i]                                   = pExposureData[i].sensitivity;
    }
    ...
    result = StatsUtil::WriteVendorTag(m_pNode,
                                       ...
                                       "AECFrameControl",
                                       reinterpret_cast<VOID*>(pFrameControlOut),
                                       sizeof(AECFrameControl));
    if (CamxResultSuccess != result)
    {
    ...
    result = StatsUtil::WriteVendorTag(m_pNode,
                                       "org.quic.camera2.statsconfigs",
                                       "AECSensitivity",
                                       reinterpret_cast<VOID*>(&aecSensitivity),
                                       sizeof(aecSensitivity));
    if (CamxResultSuccess != result)
    {
        CAMX_LOG_ERROR(CamxLogGroupAEC, "cannot write to AECSensitivity vendor tag");
    }

    result = StatsUtil::WriteVendorTag(m_pNode,
                                       "org.quic.camera2.statsconfigs",
                                       "AECIsInsensorHDR",
                                       reinterpret_cast<VOID*>(&pEngineFrameControl->frameControl.isInsensorHDRSnapshot),
                                       sizeof(UINT8));
    if (CamxResultSuccess != result)
    {
        CAMX_LOG_ERROR(CamxLogGroupAEC, "cannot write to AEC isInsensorHDRSnapshot vendor tag");
    }
    ...
    ) const
{
    ...
    Utils::Memcpy(&pHALParam->AECompensation,
    ...
    Utils::Memcpy(&pHALParam->AELock,
        m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputControlAELock),
        sizeof(pHALParam->AELock));
    Utils::Memcpy(&pHALParam->AEMode,
        m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputControlAEMode),
        sizeof(pHALParam->AEMode));
    if ((StatsAlgoRoleDefault == pStatsProcessRequestDataInfo->cameraInfo.algoRole) ||
        (StatsAlgoRoleMaster == pStatsProcessRequestDataInfo->cameraInfo.algoRole))
    {
        Utils::Memcpy(&pHALParam->AETrigger,
            m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputControlAEPrecaptureTrigger),
            sizeof(pHALParam->AETrigger));
    }
    else
    {
    ...
    Utils::Memcpy(&pHALParam->AFTrigger,
        m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputControlAFTrigger),
        sizeof(pHALParam->AFTrigger));
    Utils::Memcpy(&pHALParam->captureIntent,
        m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputControlCaptureIntent),
        sizeof(pHALParam->captureIntent));
    Utils::Memcpy(&pHALParam->controlMode,
        m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputControlMode),
        sizeof(pHALParam->controlMode));
    Utils::Memcpy(&pHALParam->controlSceneMode,
        m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputControlSceneMode),
        sizeof(pHALParam->controlSceneMode));
    Utils::Memcpy(&pHALParam->flashMode,
        m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputFlashMode),
        sizeof(pHALParam->flashMode));
    Utils::Memcpy(&pHALParam->exposureTime,
        m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputSensorExposureTime),
        sizeof(pHALParam->exposureTime));
    Utils::Memcpy(&pHALParam->sensitivity,
        m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputSensorSensitivity),
        sizeof(pHALParam->sensitivity));
    Utils::Memcpy(&pHALParam->AEAntibandingModeValue,
        m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputControlAEAntibandingMode),
        sizeof(pHALParam->AEAntibandingModeValue));

    if (NULL != m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputControlAERegions))
    {
    ...
    {
        Utils::Memcpy(&pHALParam->sensorFlashState,
            m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeFlashState),
                sizeof(pHALParam->sensorFlashState));
    }

    if (NULL != m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputControlAETargetFpsRange))
    {
        Utils::Memcpy(&pHALParam->FPSRange, m_pStatsAECPropertyReadWrite->GetPropertyData(
            AECReadTypeInputControlAETargetFpsRange),
            sizeof(pHALParam->FPSRange));
    }

    if (NULL != m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputControlZslEnable))
    {
        Utils::Memcpy(&pHALParam->controlZslEnable,
                m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputControlZslEnable),
                sizeof(pHALParam->controlZslEnable));
    }

    if (NULL != m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeSensorFrameDuration))
    {
        Utils::Memcpy(&pHALParam->frameDuration,
                m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeSensorFrameDuration),
                sizeof(pHALParam->frameDuration));
    }

    if (NULL != m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputControlPostRawSensitivityBoost))
    {
        Utils::Memcpy(&pHALParam->controlPostRawSensitivityBoost,
            m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputControlPostRawSensitivityBoost),
    ...
    if (1 == m_pStaticSettings->feature1)
    {
        pHALParam->videoHDRType = HDRTypeValues::ISPHDR;
    }
    else if (HDRTypeValues::SensorHDR == m_HDRType)
    {
            ...
            *ppPeerInfo = *(reinterpret_cast<VOID**>(pData[0]));
            CAMX_LOG_VERBOSE(CamxLogGroupAEC, "MultiCamera Pdata: PeerInfo:%010p peer_pipeline_id:%d "
                ...
                *ppPeerInfo, peerPipelineId, m_currProcessingRequestId,
                (m_currProcessingRequestId - requestDelta), m_cameraId, negates[0], requestDelta,
    ...
    UINT32*         pPartialTagCount)
{
    ...
    else
    {
        ...
        CAMX_LOG_ERROR(CamxLogGroupMeta, "ERROR cannot publish tag count (%d %d %d)",
    ...
    if (CamxResultSuccess == result)
    {
        ...
        *pPartialTagCount = NumberOfPartialMetadataTags;
        ...
        CAMX_LOG_VERBOSE(CamxLogGroupMeta, "%d tags will be published. tag count (%d %d %d)",
            tagCount,
            m_vendorTagInfoOutputputList.vendorTagCount,
            numVendorTags,
            numMetadataTags);
    ...
    AECCommandInputParam* pInput)
{
    ...
    if (NULL != pInput)
    {
        result = m_AECGyro.PopulateGyroData(pInput);
    ...
    NCSSensorType  sensorType)
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupAEC, "CAECGyro: Unable to Query caps sensor type %d error %s", sensorType,
            Utils::CamxResultToString(result));
    }
    ...
    AECCommandInputParam* pInput)
{
    ...
    if (m_pNCSSensorHandleGyro != NULL)
    {
        ...
        // Max number of gyro samples needed per frame is 1 now.
        ...
        if (NULL != pDataObj)
        {
            ...
            for (index = 0; index < pGyroInfo->sampleCount; index++)
            {
