        ...
        if (NULL != pData[0] && NULL != pData[1])
        {
            ...
            switch (pMCCInfo->currentCameraRole)
            {
                case CameraRoleTypeUltraWide:
                    pStatsProcessRequestData->cameraInfo.cameraType = StatsCameraTypeUltraWide;
                    break;
                ...
                case CameraRoleTypeDefault:
                    ...
                    break;
                default:
                    CAMX_LOG_ERROR(CamxLogGroupAWB, "Invalid camera type for request %llu",
            ...
            CAMX_LOG_VERBOSE(CamxLogGroupAWB, "AWBNode CameraInfo ReqId:%llu CamId:%d Role:%-7s Type:%-7s",
                pStatsProcessRequestData->requestId,
                pStatsProcessRequestData->cameraInfo.cameraId,
                StatsUtil::GetRoleName(pStatsProcessRequestData->cameraInfo.algoRole),
                StatsUtil::GetCameraTypeName(pStatsProcessRequestData->cameraInfo.cameraType));
    ...
    ExecuteProcessRequestData* pExecuteProcessRequestData)
{
    ...
    CAMX_LOG_VERBOSE(CamxLogGroupAWB, "AWBNode execute request for requsetId %llu pipeline %d seqId %d",
        pExecuteProcessRequestData->pNodeProcessRequestData->pCaptureRequest->requestId,
...
VOID AWBNode::InitializePreviousSessionData()
{
    ...
    CAMX_LOG_WARN(CamxLogGroupAWB, "AWBNode Camx FW to implement a mechanism to save previous session data");
