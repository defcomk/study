    ...
    UINT64                      requestIdOffsetFromLastFlush)
{
    ...
    result = ParseMultiRequestInfo(pExecuteProcessRequestData, pStatsRequestData);
    ...
    StatsProcessRequestData*    pStatsRequestData)
{
    ...
    if ((numOfActivePipeline <= MaxActiveRealtimePipeline) && (numOfActivePipeline > 0) && (TRUE == foundOwnPipeline))
    {
        if (TRUE == isMaster)
        {
            ...
            if (FirstValidRequestId != pMultiReqSync->syncSequenceId)
            {
            ...
            else
            {
                ...
                pCaptureRequest->peerSyncInfo.needSync = FALSE;
                result = CamxResultSuccess;
                CAMX_LOG_VERBOSE(CamxLogGroupStats, "No need to add dependency: seqID:%llu Prev Multi:%d",
                                 pMultiReqSync->syncSequenceId,
