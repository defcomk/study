...
static const UINT NumberOfPartialMetadataTags = 5;
...
{
    extern const CHAR* pDefaultAlgorithmPath;
    ...
    const StatsInitializeData* pInitializeData)
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        AFCalibrationOTPData  afCalibrationOTPData                 = {0};
        ...
        m_pAFIOUtil->GetOTPData(&afCalibrationOTPData);
        createParams[AFCreateParamAFCalibrationOTPData].createParamType = AFCreateParamAFCalibrationOTPData;
        createParams[AFCreateParamAFCalibrationOTPData].pParam          = static_cast<VOID*>(&afCalibrationOTPData);
    ...
    StatsCameraInfo* pStatsCameraInfo)
{
    ...
    ChiCameraInfo mCameraInfo;
    mCameraInfo.pLegacy = static_cast<VOID*>(CAMX_CALLOC(sizeof(mCameraInfo)));
    pInitializeData->pChiContext->GetCameraInfo(cameraId, &mCameraInfo);

    if (NULL != mCameraInfo.pLegacy)
    {
        CAMX_FREE(mCameraInfo.pLegacy);
        mCameraInfo.pLegacy = NULL;
    }

    m_isFixedFocus = mCameraInfo.lensCaps.isFixedFocus;

    if ((TRUE == pStaticSettings->multiCameraEnable) && (MultiCamera3ASyncDisabled != pStaticSettings->multiCamera3ASync))
    {
        switch (mCameraInfo.sensorCaps.positionType)
        {
            case  REAR:
                {
                    pStatsCameraInfo->cameraType = StatsCameraTypeWide;
                    pStatsCameraInfo->algoRole = StatsAlgoRoleMaster;
                    break;
                }
            case REAR_AUX:
                {
                    pStatsCameraInfo->cameraType = StatsCameraTypeTele;
                    pStatsCameraInfo->algoRole = StatsAlgoRoleSlave;
                    break;
                }
            default:
                {
    ...
    const StatsProcessRequestData* pStatsProcessRequestDataInfo)
{
    ...
    m_requestId                           = pStatsProcessRequestDataInfo->requestId;
    ...
    CAMX_LOG_INFO(CamxLogGroupAF, "AF Processor execute request for id %llu requestIdOffsetFromLastFlush  %llu",
    ...
    else
    {
        if (TRUE == pStaticSettings->enableNCSService)
        {
            ...
            if (result == CamxResultEFailed)
            {
                ...
                CAMX_LOG_WARN(CamxLogGroupAF, "Failed to populate Gyro data");
            }
            ...
            {
                /// In case of error, just print log and proceed
                CAMX_LOG_WARN(CamxLogGroupAF, "Failed to populate Gravity data");
            }
        ...
        if (CamxResultSuccess == result)
        {
            ...
            {
                m_pAFIOUtil->GetAFReadProperties();
                ProcessSetParams(
...
CAFStatsProcessor::~CAFStatsProcessor()
{
    ...
    if (NULL != m_pAFAlgorithm)
    {
        ...
        UINT* pOverrideCameraClose                                             = (UINT *)&pStaticSettings->overrideCameraClose;
        destroyParams[AFDestroyParamTypeCameraCloseIndicator].destroyParamType = AFDestroyParamTypeCameraCloseIndicator;
        destroyParams[AFDestroyParamTypeCameraCloseIndicator].pParam           = static_cast<VOID*>(pOverrideCameraClose);
    ...
    const StatsProcessRequestData* pStatsProcessRequestDataInfo)
{
    ...
    UnstabilizedFaceROIInformation unstabilizedFaceROIInfo = {};
    AFROIInfo             AFROIInfo                        = {};
    StatsCameraInfo       cameraInfo                       = {};
    ...
    if (CamxResultSuccess == m_pAFIOUtil->ReadFaceROIInformation(&faceROIInfo, &unstabilizedFaceROIInfo))
    {
        ...
        setParam[setParamList.numberOfSetParam].sizeOfSetParam = sizeof(UnstabilizedFaceROIInformation);
        setParamList.numberOfSetParam = setParamList.numberOfSetParam + 1;
    }
    ...
    AFAlgoInputList* pInputList)
{
    ...
    m_AFLock = m_pAFIOUtil->GetAFLock();
    UpdateInputParam(AFInputTypeLock, sizeof(BOOL), static_cast<VOID*>(&m_AFLock), inputCount++);
