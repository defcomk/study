    ...
    const UINT64             requestId)
{
    ...
    for (UINT32 i = 0; i < maxIndex; i++)
    {
        ...
        switch (pOutput->pAFDOutputList[i].outputType)
        {
            case AFDAlgoOutputRowSumConfig:
                {
                    ...
                    CAMX_LOG_VERBOSE(CamxLogGroupStats, "statsConfig RequestId=%llu Hnum:%u Vnum:%u",
                        requestId,
            ...
            default:
                {
                    CAMX_LOG_ERROR(CamxLogGroupStats, "Invalid output type %u!", pOutput->pAFDOutputList[i].outputType);
                    break;
                }
        }
    }
    ...
    const UINT64             requestId)
{
    ...
    for (UINT32 i = 0; i < maxIndex; i++)
    {
        ...
        switch (pOutput->pAFDOutputList[i].outputType)
        {
            case AFDAlgoOutputFrameSetting:
                {
                    ...
                    CAMX_LOG_VERBOSE(CamxLogGroupStats, "RequestId=%llu Detected AFD Mode - %d",
            ...
            default:
                {
                    CAMX_LOG_ERROR(CamxLogGroupStats, "Invalid output type!");
                    break;
                }
        }
    }
...
AFDSensorInfo* CAFDIOHandler::ReadSensorInput()
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupStats, "unable to read PropertyIDUsecaseSensorCurrentMode index");
    }
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupStats, "unable to read PropertyIDUsecaseSensorModes pSensorModes");
    }
    ...
    CAMX_LOG_VERBOSE(CamxLogGroupStats, "binning(H,V):(%d %d) maxFPS:%f maxLC:%d minHblanking:%d minVblanking:%d"
    ...
    AFDAECInfo* pAFDAECInfo)
{
    if (NULL == pAFDAECInfo)
    {
        CAMX_LOG_ERROR(CamxLogGroupStats, "AEC info NULL pointer");
        ...
        case PropertyIDISPHDRBEConfig:
            {
                ...
                {
                    CAMX_LOG_ERROR(CamxLogGroupStats, "unable to read PropertyIDISPHDRBEConfig BGStatsConfig");
        ...
        case PropertyIDParsedHDRBEStatsOutput:
            {
                ...
                {
                    CAMX_LOG_ERROR(CamxLogGroupStats, "unable to read PropertyIDParsedHDRBEStatsOutput BGStatsConfig");
                    result = CamxResultENoSuch;
                }
                break;
            }
        case PropertyIDParsedRSStatsOutput:
            {
                ...
                {
                    CAMX_LOG_ERROR(CamxLogGroupStats, "PropertyIDParsedRSStatsOutput is NULL");
                    result = CamxResultENoSuch;
                }
                break;
            }
        case PropertyIDISPRSConfig:
            {
                ...
                {
                    CAMX_LOG_ERROR(CamxLogGroupStats, "PropertyIDISPRSConfig is NULL");
                    result = CamxResultENoSuch;
                }
                break;
            }
        default:
            CAMX_LOG_ERROR(CamxLogGroupStats, "Unsupported PropertyId %d", propertyId);
    ...
    StatsRowSum* pRowSumConfig)
{
    if (NULL == pRSConfig)
    {
        CAMX_LOG_ERROR(CamxLogGroupStats, "pRowSum stats NULL pointer");
        return CamxResultEInvalidPointer;
    }
    ...
    pRowSumConfig->regionHeight = pRSStatsConfig->RSConfig.statsRgnHeight;
    pRowSumConfig->regionWidth = pRSStatsConfig->RSConfig.statsRgnWidth;

    CAMX_LOG_VERBOSE(CamxLogGroupStats, "HRC:%d VRC:%d rh:%d rw:%d",
...
{
    pRowSumInfo->rowSum = reinterpret_cast<UINT32*>(pRowSum);
    ...
    StatsBayerGrid* pBGStat)
{
    if (NULL == pBGStat)
    {
        CAMX_LOG_ERROR(CamxLogGroupStats, "BG stats NULL pointer");
        return CamxResultEInvalidPointer;
    }
    ...
    pBGStat->bitDepth              = IFEPipelineBitWidth;
    ...
    UINT* pSensorCurrentMode)
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupStats, "Sensor Current Mode NULL pointer");
    }
}
    ...
    AFDSensorInfo* pSensorInfo)
{
    if (NULL == pSensorInfo)
    {
        CAMX_LOG_ERROR(CamxLogGroupStats, "Sensor Info NULL pointer");
    }
    ...
    AFDAECInfo* pAFDAECInfo)
{
    ...
    CAMX_LOG_VERBOSE(CamxLogGroupStats, "AEC Settled?? at AFD %d", pAECFrameInfo->AECSettled);
    ...
    AFDAFInfo* pAFInfo)
{
    if (NULL == pAFInfo)
    {
        CAMX_LOG_ERROR(CamxLogGroupStats, "AF info NULL pointer");
        return CamxResultEInvalidPointer;
    }
    ...
    AFDHALParam* pHALParam)
{
    ...
    switch(pHALParam->antiBandingMode)
    {
        ...
        default:
            ...
            CAMX_LOG_ERROR(CamxLogGroupStats, "Invalid antibanding type");
            break;
    }
