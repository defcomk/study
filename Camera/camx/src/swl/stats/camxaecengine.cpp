    ...
    switch (command)
    {
        case AECCommand::ProcessStats:
            ...
            pOutput->pProcessStatsOutput->algorithmOutput.engineFrameControl.exposureData[ExposureIndexSafe].exposureTime
                = m_HALParam.exposureTime;
            CAMX_LOG_INFO(CamxLogGroupStats, " HAL exposureTime: %ld", m_HALParam.exposureTime);
            break;
        case AECCommand::StopDriver:
    ...
    AECCommandOutputParam*  pOutput)
{
    ...
    switch (m_LEDCalibrationState)
    {
        ...
        case LEDCalibrationState::Complete:
            ...
            break;
        default:
            CAMX_LOG_INFO(CamxLogGroupStats, "Not handled AECLEDCalibration state %d Command %d", m_AECPreFlashState, command);
            break;
    }
    return result;
}
    ...
    AECCommandOutputParam*  pOutput)
{
    ...
    switch (command)
    {
        ...
        case AECCommand::SetPerFrameControlParam:
            ...
            break;
        case AECCommand::SetNodesUpdate:
            if (IsAFFocused(pInput->pNodesUpdate))
            {
    ...
    AECCommandOutputParam*  pCommandOutput)
{
    ...
    if (NULL != m_pStaticSettings)
    {
        ...
        else
        {
            ...
            CAMX_LOG_VERBOSE(CamxLogGroupAEC, "Initial Frames or snapshot frames: StatisticsStartUpInvalid");

            if (statsInfo.requestId == m_maxPipelineDelay)
            {
                AECCommandInputParam  inputParam = { 0 };
                AECCommandOutputParam outputParam = { 0 };
                AECEngineFrameControl engineFrameControl = {};

                inputParam.pInitStreamConfig = &m_statsStreamInitConfig;
                outputParam.pFrameControl = &engineFrameControl;
                result = HandleCommand(AECCommand::StartStreaming, &inputParam, &outputParam);
            }
        }
        ...
        // Set Chi Stats Request ID
        CAECEngineUtility::SetAlgorithmInputEntry(&algorithmInput[0],
                                                  AECAlgoInputStatsRequestID,
                                                  sizeof(UINT64),
                                                  &pCommandInput->pAlgorithmInput->requestId);
        ...
        CAECEngineUtility::SetAlgorithmOutputEntry(&algorithmOutput[0],
                                                   ...
                                                   &(pEngineOutput->engineAdditionalControl));
...
CamxResult CAECEngine::OverrideAlgoSetting()
{
    ...
    if (NULL != m_pStaticSettings)
    {
        ...
        // AEC Warm Start enable/disable override setting
        overrideSetting.enableWarmStart = m_pStaticSettings->enableWarmStart;
    ...
    AECEngineFrameControl*   pFrameControl)
{
    ...
    CAMX_LOG_INFO(CamxLogGroupAEC, "StartStreaming: Type = %d", aeOperationMode);
    ...
    if (CamxResultSuccess == result)
    {
        /* If engine has to configure some specific start up exposure, lets set that first before we get initial gains
           Required for Fast AEC for now */
        if (AECStartupModeFastConvergence == pCommandInput->pInitStreamConfig->aecStartSensitivity.startMode)
        {
            if (0 != pCommandInput->pInitStreamConfig->aecStartSensitivity.sensitivity[AECAlgoExposureShort] &&
                0 != pCommandInput->pInitStreamConfig->aecStartSensitivity.sensitivity[AECAlgoExposureLong] &&
                0 != pCommandInput->pInitStreamConfig->aecStartSensitivity.sensitivity[AECAlgoExposureSafe])
            {
    ...
    AECCommandOutputParam* pCommandOutput)
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        UnstabilizedFaceROIInformation unstabilizedFaceROIInformation;
        ...
        // Set WarmStart settings to Algo
        if ((FALSE == m_warmStartDone))
        {
            SetWarmStartSettings(pHALParam, &statsStreamInitConfig);
            CAECEngineUtility::LoadSetParamList(&setParam[index],
                &statsStreamInitConfig.aecStartSensitivity,
                AECAlgoSetParamStartUpExpSensitivity,
                sizeof(statsStreamInitConfig.aecStartSensitivity));
            index++;
    ...
    AECEngineFrameControl*    pFrameControl)
{
    ...
    const UINT32          numberOfOutputs   = 4;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// CAECEngine::IsAFFocused
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
BOOL CAECEngine::IsAFFocused(
    AECEngineNodesUpdate* pNodesUpdate)
{
    BOOL focused = FALSE;
    if (TRUE == m_AECPreFlashSkipChecking)
    {
        CAMX_LOG_VERBOSE(CamxLogGroupAEC, "Skipped checking IsAFFocused");
        m_AECPreFlashSkipChecking = FALSE;
    }
    else
    {
...
CamxResult CAECEngine::GetFlashSnapshotGains()
{
    ...
    CAMX_LOG_VERBOSE(CamxLogGroupAEC,
                     ...
                     " FlashType=%d LEDInfluenceRatio=%f RG:BG Ratio=%f:%f LED 1:2=%d:%d",
...
CamxResult CAECEngineUtility::SetUniformFaceROIToSetParamList(
    AECAlgoSetParam*                pSetParam,
    AECEngineHALParam*              pHALParam,
    UnstabilizedFaceROIInformation* pUnstabilizedFaceROIInformation,
    StatsSensorInfo*                pStatsSensorInfo)
{
    CamxResult result = CamxResultSuccess;
    if ((NULL == pSetParam) ||
    ...
    if (CamxResultSuccess == result)
    {
        ...
        pSetParam->sizeOfAECSetParam = sizeof(UnstabilizedFaceROIInformation);
    }
...
{
    CAMX_LOG_VERBOSE(CamxLogGroupStats, "inside StoreLEDCalibrationData mode is %d",
    ...
    else
    {
        ...
        CAMX_LOG_ERROR(CamxLogGroupStats, "Error! (%s) Failed to write the dual LED calibration data to file", errnoStr);
    }
}
...
CamxResult CAECEngine::LoadLEDCalibrationData()
{
    ...
    if (NULL != pInFile)
    {
        ...
        if (data.count == 0)
        {
            CAMX_LOG_ERROR(CamxLogGroupStats, "No dual LED calibration data to read");
        }
        else
        {
                ...
                NULL != data.pBGRatios)
            {
                ...
                if (CamxResultSuccess != result)
                {
                    CAMX_LOG_ERROR(CamxLogGroupStats, "Error! Failed to set the Dual LED calibration data to the algorithm");
                }
    ...
    {
        CAMX_LOG_VERBOSE(CamxLogGroupStats, "Dual LED calibration data file not present!");
    }
...
CamxResult CAECEngine::GetLEDCalibrationConfig()
{
    ...
    if (AECAlgoFlashMeasKindOff == m_LEDCalibration.mode)
    {
        CAMX_LOG_VERBOSE(CamxLogGroupStats, "Dual LED Calibration is disabled");
    }
    else
    {
        CAMX_LOG_VERBOSE(CamxLogGroupStats, "Dual LED Calibration is enabled for %s",
    ...
    AECEngineProcessStatsOutput* pOutput)
{
    ...
    if (LEDCalibrationState::Ready == GetLEDCalibrationState())
    {
        ...
        {
            CAMX_LOG_VERBOSE(CamxLogGroupStats, "creating table");
            ...
            m_LEDCalibration.completedCount < m_LEDCalibration.config.measurementsCount)
        {
            ...
            CAMX_LOG_VERBOSE(CamxLogGroupStats, "currents %d %d", pFrameControl->LEDCurrents[0], pFrameControl->LEDCurrents[1]);
        }
    ...
    {
        CAMX_LOG_VERBOSE(CamxLogGroupStats, "2:m_LEDCalibrationState is %d!", m_LEDCalibrationState);
        pFrameControl->frameControl.flashState = AECAlgoFlashStateCalibaration;
        AECAlgoGetParamOutput outputList[1];
        ...
        CAMX_LOG_VERBOSE(CamxLogGroupStats, "1:m_LEDCalibration.result.status is %d! result %d",
        ...
        {
            CAMX_LOG_VERBOSE(CamxLogGroupStats, "3:m_LEDCalibrationState is %d  FlashPoint result status is %d!",
            ...
            {
                CAMX_LOG_VERBOSE(CamxLogGroupStats, "Dual LED calibration succeeded!");
                StoreLEDCalibrationData();
            }
            else
            {
                CAMX_LOG_ERROR(CamxLogGroupStats, "ERROR! Dual LED calibration failed");
            }
        ...
        else
        {
            ...
            CAMX_LOG_VERBOSE(CamxLogGroupStats, "Dual LED calibration measurement on going for measurement %d status %d",
                idx,
                ...
                m_LEDCalibration.result.pFlashPoints[idx].status)
            {
                ...
                CAMX_LOG_VERBOSE(CamxLogGroupStats, "node completed count %d set to match core completed count %d",
                    ...
                    m_LEDCalibration.completedCount < m_LEDCalibration.config.measurementsCount)
                {
                    m_LEDCalibration.pLEDCurTable[m_LEDCalibration.completedCount * 2] = pFrameControl->LEDCurrents[0];
                    m_LEDCalibration.pLEDCurTable[m_LEDCalibration.completedCount * 2 + 1] = pFrameControl->LEDCurrents[1];
                    CAMX_LOG_VERBOSE(CamxLogGroupStats, "currents %d %d",
                ...
                m_LEDCalibration.result.pFlashPoints[idx].status)
            {
                    ...
                    m_LEDCalibration.completedCount < m_LEDCalibration.config.measurementsCount)
                {
                    ...
                    CAMX_LOG_VERBOSE(CamxLogGroupStats, "Ongoing calibration, setting currents: %d %d",
                        pFrameControl->LEDCurrents[0], pFrameControl->LEDCurrents[1]);
                }
                else
                {
                    CAMX_LOG_INFO(CamxLogGroupStats, "completedCount %d measurementsCount %d",
                        m_LEDCalibration.completedCount, m_LEDCalibration.config.measurementsCount);
                }
            }
        }
    ...
    CAMX_LOG_VERBOSE(CamxLogGroupStats, " flashState is %d!, calibFlashState %d!",
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// CAECEngine::SetWarmStartSettings
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID CAECEngine::SetWarmStartSettings(
    const AECEngineHALParam* pHALParam,
    StatsStreamInitConfig*   pStatsStreamInitConfig)
{
    CamX::Utils::Memset(pStatsStreamInitConfig, 0, sizeof(StatsStreamInitConfig));

    pStatsStreamInitConfig->operationMode = StatsOperationModeNormal;

    if ((0 != pHALParam->warmStartSensitivity[AECAlgoExposureShort]) &&
        (0 != pHALParam->warmStartSensitivity[AECAlgoExposureLong]) &&
        (0 != pHALParam->warmStartSensitivity[AECAlgoExposureSafe]))
    {
        pStatsStreamInitConfig->aecStartSensitivity.startMode                         = AECStartupModeWarmStart;
        pStatsStreamInitConfig->aecStartSensitivity.sensitivity[AECAlgoExposureShort] =
            pHALParam->warmStartSensitivity[AECAlgoExposureShort];
        pStatsStreamInitConfig->aecStartSensitivity.sensitivity[AECAlgoExposureLong]  =
            pHALParam->warmStartSensitivity[AECAlgoExposureLong];
        pStatsStreamInitConfig->aecStartSensitivity.sensitivity[AECAlgoExposureSafe]  =
            pHALParam->warmStartSensitivity[AECAlgoExposureSafe];
    }

    CAMX_LOG_VERBOSE(CamxLogGroupStats, "warmstart (shortG:%f longG:%f safeG:%f)",
        pHALParam->warmStartSensitivity[AECAlgoExposureShort],
        pHALParam->warmStartSensitivity[AECAlgoExposureLong],
        pHALParam->warmStartSensitivity[AECAlgoExposureSafe]);
}
