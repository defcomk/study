    ...
    VOID*                                                 pOutput)
{
        ...
        (NULL != pOutput))
    {
        ...
        pUnpackedField->data_collect_en         = 1;
            ...
            pInput->pAdrcInputData->ltmEnable)
        {
            ...
            if (SWTMCVersion::TMC11 == pInput->pAdrcInputData->version)
            {
                adrc_ltm_mix_rate = IQSettingUtils::ClampFLOAT(
                                          2.0f - (pInput->pAdrcInputData->kneePoints.KneePointsTMC11.kneeY[1] /
                                                  pInput->pAdrcInputData->kneePoints.KneePointsTMC11.kneeX[1]),
                                          0.0f,
                                          1.0f);
                ...
                {
                    // PCHIP curve
                    IQSettingUtils::PCHIPCurve(&pInput->pAdrcInputData->kneePoints.KneePointsTMC11.kneeX[0],
                                               &pInput->pAdrcInputData->kneePoints.KneePointsTMC11.kneeY[0],
                                               &pInput->pAdrcInputData->pchipCoef[0],
                ...
                {
                    // Bezier curve
                    IQSettingUtils::BezierCurve(&pInput->pAdrcInputData->kneePoints.KneePointsTMC11.kneeX[0],
                                                &pInput->pAdrcInputData->kneePoints.KneePointsTMC11.kneeY[0],
                                                inverseGammaInNormalized,
                                                tmcOutLtmGainCurve,
                                                LTM_LUT_SIZE);
    ...
    FLOAT*        pLtmCurve)
{
    ...
    {
        out_indx[i] =
            IQSettingUtils::ClampFLOAT(((pGain[i] / (1 << LTM_SCALE_Q)) * pInEqvlnt[i]) * LTM_LUT_SIZE / LTM_MAX_VALUE,
            0.0f, 64.0f);
    }
