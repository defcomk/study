    ...
    VOID*                                                 pOutput)
{
    ...
    if ((NULL != pInput)        &&
        ...
        (NULL != pOutput))
    {
        IPEANRRegCmd*  pRegPass = static_cast<IPEANRRegCmd*>(pOutput);
        AnrParameters*       pFWStruct                   = static_cast<AnrParameters*>(pInput->pANRParameters);
        UINT32               numOfPasses                 = pInput->numPasses;
        UINT32               imageWidthPixelsWithMargin  = pInput->pImageDimensions->widthPixels;
        UINT32               imageHeightPixelsWithMargin = pInput->pImageDimensions->heightLines;
        UINT32               imageWidthPixels            = (NULL != pInput->pMarginDimensions) ?
             imageWidthPixelsWithMargin - pInput->pMarginDimensions->widthPixels : imageWidthPixelsWithMargin;
        UINT32               imageHeightPixels           = (NULL != pInput->pMarginDimensions) ?
             imageHeightPixelsWithMargin - pInput->pMarginDimensions->heightLines : imageHeightPixelsWithMargin;
        NCLIB_CONTEXT_ANR    nclibContext;
        ANR_Chromatix*       pAnrChromatix               = static_cast<ANR_Chromatix*>(pInput->pNCChromatix);
        UINT32               mE                          = pInput->bitWidth - 8;

        IQSettingUtils::Memset(pAnrChromatix, 0x0, sizeof(ANR_Chromatix)*pInput->numPasses);
        Clear_ANR(pRegPass, pInput->numPasses);
        ...
        MapChromatix2ANRChromatix(pReserveData->mod_anr10_pass_reserve_data,
                                  pData,
                                  numOfPasses,
                                  pAnrChromatix);

        nclibContext.frameNumber      = static_cast<uint32_t>(pInput->frameNum);

        UpdateGeometryParameters(pInput, &nclibContext);

        CAMX_LOG_VERBOSE(CamxLogGroupIQMod, "numOfPasses %d, w %d, h %d, marginw %d, marginH %d",
                         numOfPasses, imageWidthPixels, imageHeightPixels,
                         imageWidthPixelsWithMargin, imageHeightPixelsWithMargin);
        intRet = ANR_FlowProcessNcLib(pAnrChromatix, &nclibContext, pRegPass, pFWStruct, numOfPasses,
                                      imageWidthPixels, imageHeightPixels, imageWidthPixelsWithMargin,
                                      imageHeightPixelsWithMargin, pInput->bitWidth);
        if (NC_LIB_SUCCESS != intRet)
        {
            result = FALSE;
            CAMX_LOG_ERROR(CamxLogGroupIQMod, "ANR_FlowProcessNcLib failed %d", result);
        }
        else
        {
            CAMX_LOG_VERBOSE(CamxLogGroupIQMod, "ANR10Setting::CalculateHWSetting success");

            if (TRUE == pInput->validateANRSettings)
            {
                result = ANR_ValidateNcLibRegs(pRegPass, numOfPasses, mE);

                if (0 != result)
                {
                    CAMX_LOG_VERBOSE(CamxLogGroupIQMod, "Validation of registers returned failure %d", result);
                }
                else
                {
                    CAMX_LOG_VERBOSE(CamxLogGroupIQMod, "Validation of registers returned success %d", result);
                }
                // Only validation if settings is enabled
                result = TRUE;
            }
        }
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID ANR10Setting::MapChromatix2ANRChromatix(
