    ...
    INT32*  comp_num_bot_extended_pels)
{
    ...
    else
    {
        for (i = 0; i < MaxYorUVChannels; i++)
        {
            ...
            right      = clean_frac(comp_init_phase_x[i]) + (comp_out_width[i] - 1) * comp_phase_step_x[i] -
                (comp_in_width[i] - 1) * FixPixelUnitScale + num_taps_x * FixPixelUnitScale;
            right      = -right;
            ...
            bot        = clean_frac(comp_init_phase_y[i]) + (comp_out_height[i] - 1) * comp_phase_step_y[i] -
              (comp_in_height[i] - 1) * FixPixelUnitScale + static_cast<INT32>(num_taps_y) * FixPixelUnitScale;
            bot        = -bot;
    ...
    UINT32* pInputROIVerOffset)
{
    ...
    *pYPreloadH  = static_cast<UINT8>(
        RAU_alignment_h[0] + leftPEOverfetch[0]- pCompNumLeftExtendedPels[0] + HorizontalNum2DFilterTaps + 2); // alpha will be y_preload_h - 2
    *pYPreloadV  = static_cast<UINT8>(
        RAU_alignment_v[0] + topPEOverfetch[0] - pCompNumTopExtendedPels[0] + VerticalNum2DFilterTaps); //alpha will be y_preload_v - 1
    *pUVPreloadH = static_cast<UINT8>(
        RAU_alignment_h[1] + leftPEOverfetch[1]- pCompNumLeftExtendedPels[1] + HorizontalNum2DFilterTaps + 2);
    *pUVPreloadV = static_cast<UINT8>(
        RAU_alignment_v[1] + topPEOverfetch[1] - pCompNumTopExtendedPels[1] + VerticalNum2DFilterTaps);
