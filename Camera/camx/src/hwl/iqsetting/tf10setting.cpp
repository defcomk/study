    ...
    VOID*                                               pOutput)
{
    ...
    {
        IPETFRegCmd*    pRegPass = static_cast<IPETFRegCmd*>(pOutput);
        ...
        LmcParameters lmcParameters; // RTODO: temporary, needs to be fixed
        ...
        IQSettingUtils::Memset(&lmcParameters, 0x0, sizeof(lmcParameters));
        Clear_TF(pRegPass, PASS_NAME_MAX);
        ...
        for (UINT32 pass = 0; pass < pInput->maxUsedPasses; pass++)
        {
        ...
        nclibContext.frameNumber                  = pInput->mfFrameNum;
        ...
        if (TRUE == result)
        {
            ...
            intRet = TF_ProcessNcLibFull(pTFChromatix,
                                         &nclibContext,
                                         pInput->maxUsedPasses,
                                         pInput->perspectiveConfidence,
                                         pRegPass,
                                         pFwRefineParams,
                                         pFwTFParams,
                                         &lmcParameters);
...
{
    pData->TF10ChromatixSize  = (sizeof(TF_Chromatix) * PASS_TYPE_NUM);
