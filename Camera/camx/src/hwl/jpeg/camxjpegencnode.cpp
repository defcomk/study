...
// @brief list of metadata tags published by the JPEG node
static const UINT32 JPEGOutputTags[] =
{
...
CamxResult JPEGEncNode::PostPipelineCreate()
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        CAMX_LOG_VERBOSE(CamxLogGroupJPEG, "Rotation GPU %d, FCV %d",
                         m_rotationHandledGPU, m_rotationHandledFCV);
        ...
        else
        {
            if (TRUE == this->m_bThumbnailEncode)
            {
                ...
                if ((NULL != pStaticCaps) && (0 < pStaticCaps->numJPEGThumbnailSizes))
                {
                    ...
                    {
                        CAMX_LOG_ERROR(CamxLogGroupJPEG, "Aspect ratio for thumbanil is not matched");
                    }
                }

                optimalDimension.width  = m_thumbnailWidthMax  * OptimalThumbnailScalingFactor;
                optimalDimension.height = m_thumbnailHeightMax * OptimalThumbnailScalingFactor;
...
{
    UINT32 numJpegTags = CAMX_ARRAY_SIZE(JPEGOutputTags);

    for (UINT32 tagIndex = 0; tagIndex < numJpegTags; ++tagIndex)
    {
        pPublistTagList->tagArray[tagIndex] = JPEGOutputTags[tagIndex];
    }

    pPublistTagList->tagCount = numJpegTags;
    CAMX_LOG_VERBOSE(CamxLogGroupMeta, "%u tags will be published", numJpegTags);
