...
static const UINT32 MultiCameraIdRoleIndex      = 4;
...
/// @brief IFE Bandwidth AB votes needed for the active use case
struct IFEResourceBWConfigAB
{
    UINT32 usageType;          ///< Single IFE or Dual IFE mode
    UINT32 numRdi;             ///< Number of RDIs
    UINT64 leftPixelVoteAB;    ///< AB Bandwidth vote for "left" IFE, valid for single IFE
    UINT64 rightPixelVoteAB;   ///< AB Bandwidth vote for "right" IFE, valid for dual IFEs
    UINT64 rdiVoteAB[4];       ///< AB Bandwidth vote for RDI path
} CAMX_PACKED;
...
CAMX_STATIC_ASSERT_MESSAGE((sizeof(IFEResourceBWConfigAB) % 8) == 0,    "IFEResourceBWConfig 64b-aligned");
...
private:
    ...
    ///
    /// @param  inputWidthInPixels        Input width IFE will use for the desired port. This should account for any applied
    ///                                   crop as well.
    /// @param  outputWidthInBytes        Output width for the desired port in bytes
    /// @param  IFEIndex                  For dual-IFE: 0==left; 1==right; 2=single-IFE
    ///
    /// @return Line length in micro seconds
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    /// @param  pBwResourceConfig           Contains both Uncompressed BW inside camnoc and Compressed BW outside camnoc,
    ///                                     used to config MNOC and the rest of the path to DDR
    /// @param  pBwResourceConfigAB         Contains AB votes for Compressed BW outside camnoc,
    ///                                     used to config AB vote for MNOC
    /// @param  requestId                   RequestId for which BW config is being setup
    ///
    /// @return return CamxResult
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    CamxResult CalculateActiveBandwidth(
        ExecuteProcessRequestData* pExecuteProcessRequestData,
        IFEResourceBWConfig*       pBwResourceConfig,
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /// CalculateRAWBandwidth
    ///
    /// @brief  Calculates the BW required for RAW ports to operate the IFE for the current use case.
    ///
    /// @param  IFEIndex         Index of the IFE being used
    /// @param  portId           Port Id for which bandwidth calculation is required
    /// @param  pCamnocLineBW    Pointer to cumulative line bandwidth computed for CAMNOC
    /// @param  pExternalLineBW  Pointer to cumulative line bandwidth computed for MNOC
    /// @param  requestId        RequestId of the frame being processed
    ///
    /// @return None
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    VOID CalculateRAWBandwidth(
        UINT    IFEIndex,
        UINT32  portId,
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /// CalculateRDIBandwidth
    ///
    /// @brief  Calculates the BW required for RDI ports to operate the IFE for the current use case.
    ///
    /// @param  IFEIndex            Index of the IFE being used
    /// @param  pBwResourceConfig   Contains both Uncompressed BW inside camnoc and Compressed BW outside camnoc,
    ///                             used to config MNOC and the rest of the path to DDR
    ...
    /// @param  pBytesPerPixel      Pointer to indicate bytes per pixel for the corresponding output
    /// @param  requestId           RequestId of the frame being processed
    ///
    /// @return CamxResult          True on successful execution else False on failure
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    CamxResult CalculateRDIBandwidth(
        UINT                 IFEIndex,
        IFEResourceBWConfig* pBwResourceConfig,
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /// SetupFlushHaltConfig
    ///
    /// @brief  Setup Flush Halt Register configuration
    ///
    /// @param  value of flush Halt
    ///
    /// @return CamxResult True on successful execution else False on failure
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    CamxResult SetupFlushHaltConfig(
        UINT32 value);
    ...
    /// IsUBWCLossyNeeded
    ///
    /// @brief  Check if need UBWC Format to be Lossy. For now IFE UBWC lossy output is only configured for Moorea target.
    ///         It is enabled only for resolutions >= 4K UHD, based on system team recommendation.
    ///
    /// @param  pImageFormat    Pointer to outputPort Image format to access format and dimension info
    ///
    /// @return TRUE if need UBWC format to be Lossy, FALSE otherwise
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    CAMX_INLINE BOOL IsUBWCLossyNeeded(
        const ImageFormat* pImageFormat)
    {
        const PlatformStaticCaps* pStaticCaps    = HwEnvironment::GetInstance()->GetPlatformStaticCaps();
        BOOL                      bNeedUBWCLossy = FALSE;
        if ((CSLCameraTitanChipVersion::CSLTitan175V120 == Titan17xContext::GetTitanChipVersion()) &&
            (TRUE == ImageFormatUtils::IsUBWC(pImageFormat->format))                               &&
            (3 == ImageFormatUtils::GetUBWCVersion())                                              &&
    ...
    SIZE_T                      m_resourceBWConfigSizeAB;            ///< Resource AB Bw config structure size
    IFEResourceBWConfig*        m_pBwResourceConfig;                 ///< Resource BW calculated values
    IFEResourceBWConfigAB*      m_pBwResourceConfigAB;               ///< Resource AB BW calculated values
    ...
    BOOL                        m_computeActiveABVotes;              ///< AB votes are computed to indicate actual bandwidth
};
...
public:
        ...
        const UINT32 maxNumOfCSLIFEPorts)
    {
        ...
        configSize = sizeof(IFEDualConfig) +
            (sizeof(IFEStripeConfig) * ((2 * maxNumOfCSLIFEPorts * CSLMaxNumPlanes) - 1));
