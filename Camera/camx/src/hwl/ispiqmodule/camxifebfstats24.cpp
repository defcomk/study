    ...
    ISPInputData* pInputData)
{
    ...
    if (NULL != pInputData)
    {
        if (NULL != pInputData->pStripingInput)
        {
            ...
            pInputData->pStripingInput->stripingInput.baf_inputv24.baf_h_scaler_en = static_cast<uint16_t>(
                pInputData->pStripeConfig->AFStatsUpdateData.statsConfig.BFStats.BFScaleConfig.BFScaleEnable);
            pInputData->pStripingInput->stripingInput.baf_inputv24.baf_fir_h1_en   = static_cast<uint16_t>(
                pInputData->pStripeConfig->AFStatsUpdateData.statsConfig.BFStats.BFFilterConfig->BFFIRFilterConfig.enable);
            pInputData->pStripingInput->stripingInput.baf_inputv24.baf_iir_h1_en   = static_cast<uint16_t>(
            ...
            UpdateROIDMITable(&hwROIConfig,
                pInputData->pStripingInput->stripingInput.baf_inputv24.baf_roi_indexLUT,
                sizeof(pInputData->pStripingInput->stripingInput.baf_inputv24.baf_roi_indexLUT));
            DownscalerConfig(pInputData, pAFConfig);

            pInputData->pStripingInput->stripingInput.baf_inputv24.mndsParam.en = 1;
            pInputData->pStripingInput->stripingInput.baf_inputv24.mndsParam.input_l =
                static_cast<uint16_t>(m_regCmd.phaseStripeConfig.hScaleImageSizeConfig.bits.H_IN + 1);
            pInputData->pStripingInput->stripingInput.baf_inputv24.mndsParam.output_l =
                static_cast<uint16_t>(m_regCmd.phaseStripeConfig.hScaleImageSizeConfig.bits.H_OUT + 1);
            pInputData->pStripingInput->stripingInput.baf_inputv24.mndsParam.pixel_offset =
                static_cast<uint16_t>(m_regCmd.phaseStripeConfig.hScalePadConfig.bits.H_SKIP_CNT);
            pInputData->pStripingInput->stripingInput.baf_inputv24.mndsParam.cnt_init =
                static_cast<uint16_t>(m_regCmd.phaseStripeConfig.hScaleStripeConfig0.bits.H_MN_INIT);
            pInputData->pStripingInput->stripingInput.baf_inputv24.mndsParam.interp_reso =
                static_cast<uint16_t>(m_regCmd.phaseStripeConfig.hScalePhaseConfig.bits.H_INTERP_RESO);
            pInputData->pStripingInput->stripingInput.baf_inputv24.mndsParam.rounding_option_h =
                static_cast<uint16_t>(m_regCmd.phaseStripeConfig.hScalePadConfig.bits.ROUNDING_PATTERN);
            pInputData->pStripingInput->stripingInput.baf_inputv24.mndsParam.input_processed_length =
                static_cast<uint16_t>(m_regCmd.phaseStripeConfig.hScalePadConfig.bits.SCALE_Y_IN_WIDTH + 1);
            pInputData->pStripingInput->stripingInput.baf_inputv24.mndsParam.phase_init =
                m_regCmd.phaseStripeConfig.hScaleStripeConfig1.bits.H_PHASE_INIT;
            pInputData->pStripingInput->stripingInput.baf_inputv24.mndsParam.phase_step =
