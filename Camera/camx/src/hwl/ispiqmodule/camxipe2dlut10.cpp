    ...
    if ((NULL != pParam) && (NULL != pParam->pCmdBufManagerParam) && (NULL != pInputData))
    {
        ...
        if (NULL != pResourceParams)
        {
            ...
            if (NULL != pBufferManagerName)
            {
                ...
                pResourceParams->resourceSize                 = IPE2DLUTLUTBufferSizeInDwords * sizeof(UINT32);;
    ...
    ISPInputData* pInputData)
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        pInputData->pBetDMIAddr = static_cast<VOID*>(outputData.pD2H0LUT);
    ...
    ) const
{
    ...
    if (NULL != pFile)
    {
        CamX::OsUtils::FPrintF(pFile, "******** IPE 2DLUT  ********\n");
        CamX::OsUtils::FPrintF(pFile, "* 2DLUT CFG [HEX] \n");
        CamX::OsUtils::FPrintF(pFile, "2DLUT.hueShift    = 0x%x\n", m_regCmd.hueShift);
        CamX::OsUtils::FPrintF(pFile, "2DLUT.satShift    = 0x%x\n", m_regCmd.satShift);
        CamX::OsUtils::FPrintF(pFile, "2DLUT.startA      = 0x%x\n", m_regCmd.startA);
        CamX::OsUtils::FPrintF(pFile, "2DLUT.startB      = 0x%x\n", m_regCmd.startB);
        CamX::OsUtils::FPrintF(pFile, "2DLUT.endA        = 0x%x\n", m_regCmd.endA);
        CamX::OsUtils::FPrintF(pFile, "2DLUT.endB        = 0x%x\n", m_regCmd.endB);
        CamX::OsUtils::FPrintF(pFile, "2DLUT.startInv    = 0x%x\n", m_regCmd.startInv);
        CamX::OsUtils::FPrintF(pFile, "2DLUT.endInv      = 0x%x\n", m_regCmd.endInv);
        CamX::OsUtils::FPrintF(pFile, "2DLUT.kbIntegeter = 0x%x\n", m_regCmd.kbIntegeter);
        CamX::OsUtils::FPrintF(pFile, "2DLUT.krIntegeter = 0x%x\n", m_regCmd.krIntegeter);
