...
FDHwNode::FDHwNode()
{
    ...
    CAMX_LOG_VERBOSE(CamxLogGroupFD, "FD Base dimensions : %dx%d isFDHWdisabled %d TitanVersion 0x%x",
                     m_baseFDHWDimension.width, m_baseFDHWDimension.height, m_bDisableFDHW,
    ...
    NodeCreateOutputData*      pCreateOutputData)
{
    ...
    CAMX_LOG_VERBOSE(CamxLogGroupFD, "deviceIndex=%d result=%s", deviceIndex, Utils::CamxResultToString(result));
    ...
    FDPropertyFrameSettings*    pFrameSettings)
{
    ...
    if (CamxResultSuccess != result)
    {
        CAMX_LOG_ERROR(CamxLogGroupFD, "failed, result=%d; signal the out fence", result);
...
CamxResult FDHwNode::AcquireDevice()
{
    ...
    FDAcquireInfo.getRawResults = TRUE;
