    ...
    BufferNegotiationData* pBufferNegotiationData)
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        for (UINT inputIndex = 0; inputIndex < numInputPort; inputIndex++)
        {
            ...
            {
                pInputBufferRequirement->maxWidth      = m_capability.maxInputWidth      * factor;
                pInputBufferRequirement->maxHeight     = m_capability.maxInputHeight     * factor;
                ...
                pInputBufferRequirement->optimalWidth  = m_capability.optimalInputWidth  * factor;
                pInputBufferRequirement->optimalHeight = m_capability.optimalInputHeight * factor;
            }
        ...
        for (UINT inputIndex = 0; inputIndex < numInputPort; inputIndex++)
        {
            if (inputPortId[inputIndex] != tarPort && inputPortId[inputIndex] != refPort)
            {
                ...
                if (inputPortId[inputIndex] == (tarPort + 1) && (LRMEInputPortREFLRMEDS2 == refPort))
                {
                    ...
                    DisableInputOutputLink(inputIndex, FALSE);
                }
                else
                {
            ...
            if (CamxResultSuccess == result)
            {
                ...
                for (UINT inputIndex = 0; inputIndex < pPerRequestPorts->numInputPorts; inputIndex++)
                {
                    ...
                    if ((NULL != pInputPort) && (NULL != pInputPort->pImageBuffer))
                    {
                        ...
                        enum CSLLRMEIO portId = (pInputPort->portId == m_selectedTARPort) ? CSLLRMETARInput : CSLLRMEREFInput;
                        UINT64 requestIdOffsetFromLastFlush = GetRequestIdOffsetFromLastFlush(requestId);
                        ...
                        if ((1 < numBatchedFrames) && (FirstValidRequestId == requestIdOffsetFromLastFlush) &&
                            (FALSE == m_ds2Enable))
                        {
                            ...
                            for (UINT portIndex = 0; portIndex < LRMEMaxInPorts; portIndex++)
                            {
                                ...
                                {
                                    CAMX_LOG_INFO(CamxLogGroupLRME,
                                        "LRME:%d reporting Input config, portId=%d, imgBuf=0x%x, hFence=%d, request=%llu",
                                        InstanceID(),
