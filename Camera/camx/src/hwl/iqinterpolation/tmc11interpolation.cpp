    ...
    tmc_1_1_0::tmc11_rgn_dataType* pTmcData)
{
    ...
    FLOAT  pchipCoef[MAX_ADRC_LUT_PCHIP_COEF_SIZE];
    ...
    drcIdx      = static_cast<UINT32>(round(log(pInput->DRCGain) / log(TMC11_UNIT_GAIN_STEP)));

    drcGain     = powf(TMC11_DRC_GAIN_POW_X, static_cast<FLOAT>(drcIdx));

    gainRatio   = (pInput->DRCGainDark - 1) * pTmcData->dark_boost_ratio + 1 + pTmcData->core_rsv_para1;

    drcGainDark = IQSettingUtils::MinFLOAT(drcGain * gainRatio, pInput->pChromatix->chromatix_tmc11_reserve.tone_max_ratio);

    drcIdx      = static_cast<UINT32>(round(log(drcGainDark) / log(TMC11_UNIT_GAIN_STEP)));
    ...
    CalculateKneePoints(kneeX, kneeY, pchipCoef);
    ...
    IQSettingUtils::Memcpy(&pInput->pAdrcOutputData->kneePoints.KneePointsTMC11.kneeY[0],
                           &kneeY[0],
                           sizeof(FLOAT) * TMC11_KNEE_POINTS_NUM);
    IQSettingUtils::Memcpy(&pInput->pAdrcOutputData->pchipCoef[0], &pchipCoef[0],
