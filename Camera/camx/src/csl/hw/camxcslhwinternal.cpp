    ...
    switch (groupId)
    {
        ...
        default :
            *pType = CSLHwInvalidDevice;
            ...
            CAMX_LOG_WARN(CamxLogGroupCSL, "Got a invalid groupId=%d", groupId);
            break;
    }
    return result;
}
    ...
    CHAR* pDeviceName, UINT32 groupId)
{
    ...
    if (fd < 0)
    {
        CHAR errnoStr[100] = {0};
        CamX::OsUtils::StrError(errnoStr, sizeof(errnoStr), errno);
        CAMX_LOG_ERROR(CamxLogGroupCSL, "Open failed for CSL Private device %s with group id=%d with error reason %s",
    ...
    else
    {
        ...
        switch (groupId)
        {
            case CAM_VNODE_DEVICE_TYPE:
                ...
                CAMX_LOG_VERBOSE(CamxLogGroupCSL, "Open Success for Camera Device Manager");
    ...
    while (NULL != pNode)
    {
        ...
        else
        {
            ...
            for (UINT i = 0; i < CamxMaxDeviceIndex; i++)
            {
                ...
                {
                    CAMX_LOG_VERBOSE(CamxLogGroupCSL, "Turning on DevH: 0x%x, hAc: 0x%x type: %d, idx: %d",
                                     phDevices[i],
        ...
        else
        {
            ...
            for (UINT i = 0; i < CamxMaxDeviceIndex; i++)
            {
                    ...
                    (static_cast<UINT>(phDevices[i]) == pAcquireddevice->cslDeviceIndex)))
                {
                    ...
                    CAMX_LOG_VERBOSE(CamxLogGroupCSL, "Turning off DevH: 0x%x, hAc: 0x%x type: %d, idx: %d",
                                     phDevices[i],
                                     pAcquireddevice->hAcquired,
                                     pAcquireddevice->deviceType,
                                     pAcquireddevice->cslDeviceIndex);
    ...
    BOOL          flush)
{
    ...
    for (UINT linkIndex = 0; linkIndex < CSLHwMaxLinksArray; linkIndex++)
    {
        ...
        {
            CAMX_LOG_INFO(CamxLogGroupCSL, "link_hdl(%d) isActive getting flushed",
                flushSession.link_hdl);
            result = pDeviceOp->Ioctl(&hwDevice, VIDIOC_CAM_CONTROL, &cmd);
            if (CamxResultSuccess != result)
            {
                CAMX_LOG_ERROR(CamxLogGroupCSL,
                    "Ioctl Failed for cancel/flush request with session = %d for CSL Sync Id = %llu "
                    "Cancel Request = %d, Flush All Request = %d, result = %d",
                    pSession->hSession,
                    syncId,
                    (flushSession.flush_type == CAM_REQ_MGR_FLUSH_TYPE_CANCEL_REQ) ? 1 : 0,
                    (flushSession.flush_type == CAM_REQ_MGR_FLUSH_TYPE_ALL) ? 1 : 0,
                    result);
                break;
            }
        }
            ...
            while (NULL != pNode)
            {
                ...
                if ((NULL != pHWDevice->deviceOp.Ioctl) && (TRUE == pHWDevice->isActive))
                {
                    ...
                    if (TRUE == flush)
                    {
                        ...
                        flushDeviceCmd.req_id     = requestId;
                    }
                    else
                    {
                    ...
                    {
                        CAMX_LOG_ERROR(CamxLogGroupCSL,
                            "Ioctl Failed for cancel/flush request with session = %d device = %d "
                            "for requestID = %llu "
                            "Cancel Request = %d, Flush All Request = %d, result = %d",
                            pSession->hSession,
                            pAcquireddevice->hAcquired,
                            requestId,
                            (flushSession.flush_type == CAM_FLUSH_TYPE_REQ) ? 1 : 0,
                            (flushSession.flush_type == CAM_FLUSH_TYPE_ALL) ? 1 : 0,
                            result);
                    }
                }
                pNode = CamX::LightweightDoublyLinkedList::NextNode(pNode);
    ...
    VOID*               pArg)
{
    ...
    if (returnCode < 0)
    {
        CHAR errnoStr[100] = {0};
        CamX::OsUtils::StrError(errnoStr, sizeof(errnoStr), errno);
        CAMX_LOG_WARN(CamxLogGroupCSL, "Ioctl failed for device %s (Type:%s, FD:%d, Index:%d) with error reason %s",
