    ...
    CSLHwsession* pSession)
{
    ...
    if (NULL != pSession)
    {
        ...
        CAMX_LOG_ERROR(CamxLogGroupCSL, "%u devices acquired", count);
    }
    ...
    const CHAR*         pDeviceName)
{
                    ...
                    if (TRUE == CSLHwSessionGetRefCount(pSession))
                    {
                        ...
                        if (CamxResultSuccess == result)
                        {
                            if (CSLInvalidHandle != *phDevice)
                            {
                                ...
                                CAMX_LOG_INFO(CamxLogGroupCSL, "DeviceAcquired: Name=%s phDevice=%p",
                                              CSLHwInternalDeviceTypeStrings[pHWDevice->deviceType], *phDevice);
                        ...
                        {
                            CAMX_LOG_ERROR(CamxLogGroupCSL, "Acquire failed for deviceName=%s, %s, index=%d, aState=%s",
                                           CSLHwInternalDeviceTypeStrings[pHWDevice->deviceType],
                                           pDeviceName, deviceIndex,
                                           CSLHwInternalDeviceStateStrings[pHWDevice->aState]);
                            CAMX_ASSERT_ALWAYS();
    ...
    if (CamxResultSuccess != result)
    {
        CAMX_LOG_ERROR(CamxLogGroupCSL, "Acquire Device failure: result=%u", result);
        CamX::OsUtils::RaiseSignalAbort();
    }
    return result;
}
                    ...
                    if (hCSL == pSession->hSession)
                    {
                        ...
                        if (hTempIndex < CSLHwMaxNumAcquiredDevices)
                        {
                            ...
                            if (NULL != (pHWDevice->deviceOp.Release))
                            {
                                ...
                                {
                                    CAMX_ASSERT_ALWAYS();
                                    CAMX_LOG_ERROR(CamxLogGroupCSL, "hCSL: %d, cslDeviceIndex %d oishandle, hDevice %d",
        ...
        if ((hTempIndex < CSLHwMaxNumSessions) && (TRUE == CSLHwInstanceGetRefCount()))
        {
            ...
            if ((hCSL == pSession->hSession) && (TRUE == CSLHwSessionGetRefCount(pSession)))
            {
                ...
                if (hTempIndex < CSLHwMaxNumAcquiredDevices)
                {
                    ...
                    if (NULL != (pHWDevice->deviceOp.AcquireHardware))
                    {
    ...
    if (CSLInvalidHandle != hCSL)
    {
        if (TRUE == CSLHwInstanceGetRefCount())
        {
            ...
            if (hCSLHwSession < CSLHwMaxNumSessions)
            {
                ...
                {
                    result = CSLHwScheduleRequest(pSession, hLink, requestId, bubble, syncMode);
                }
        ...
        if (hCSLHwSession < CSLHwMaxNumSessions)
        {
            if (TRUE == CSLHwInstanceGetRefCount())
            {
                ...
                if (TRUE == CSLHwSessionGetRefCount(pSession))
                {
                    if (hCSL == pSession->hSession)
                    {
                        result = CSLHwCancelRequest(pSession, cslSyncId, requestId, FALSE);
                    }
                    else
                    {
                        CAMX_ASSERT_ALWAYS();
            ...
            if (TRUE == CSLHwInstanceGetRefCount())
            {
                ...
                if (TRUE == pSession->bInFlush)
                {
                    CAMX_LOG_INFO(CamxLogGroupCSL,
                        "SUBMIT failed as Flush is in Progress - Session: %d, Device: %d, Packet: %d", hCSL, hDevice, hPacket);
                    result = CamxResultECancelledRequest;
                }
        ...
        if (CSLHwMaxNumSessions > hCSLHwSession)
        {
            if (TRUE == CSLHwInstanceGetRefCount())
            {
                ...
                if (hCSL == pSession->hSession)
                {
                    ...
                    CSLHwSetSessionState(pSession, CSLHwFlushState);
                    if (TRUE == CSLHwSessionGetRefCount(pSession))
                    {
                        result = CSLHwCancelRequest(pSession, lastFlushedSyncId, lastFlushedRequestId, TRUE);
                        CSLHwSessionPutRefCount(pSession);
                    }
                }
                else
        ...
        if (CSLHwMaxNumSessions > hCSLHwSession)
        {
            if (TRUE == CSLHwInstanceGetRefCount())
            {
                ...
                if (hCSL == pSession->hSession)
                {
                    ...
                    if (CSLHwFlushState == CSLHwGetSessionState(pSession))
                    {
                        CSLHwSetSessionState(pSession, CSLHwValidState);
                    }
                    result = CamxResultSuccess;
    ...
    CSLFence hFence)
{
    ...
    if (TRUE == CSLHwInstanceGetRefCount())
    {
        CAMX_LOG_VERBOSE(CamxLogGroupCSL, "Sync Destroy requested: hFence:%d result:%d", hFence);
