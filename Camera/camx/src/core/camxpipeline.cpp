...
static const UINT DefaultMaxPipelineDelay        = 3;
    ...
    PipelineProcessRequestData* pPipelineRequestData)
{
    ...
    Utils::Memset(pSequenceId, 0, sizeof(UINT32) * GetNumBatchedFrames());

    CAMX_ASSERT(GetNumBatchedFrames() >= pCaptureRequest->numBatchedFrames);
    ...
    CAMX_ASSERT(m_perRequestInfo[perRequestIdIndex].request.pStreamBuffers ==
        &m_pStreamBufferBlob[perRequestIdIndex * GetNumBatchedFrames()]);
    ...
    if (CamxResultSuccess == result)
    {
        ...
        if (TRUE == m_debugDataInitialized)
        {
        ...
        m_pCSLSyncIDToRequestId[pCaptureRequest->CSLSyncID % (MaxPerRequestInfo * GetNumBatchedFrames())] =
    ...
    if (CamxResultSuccess == result)
    {
        ...
        for (UINT nodeIndex = 0; nodeIndex  < m_nodeCount; nodeIndex++)
        {
            ...
            m_ppNodes[nodeIndex]->SetupRequest(pPerBatchedFrameInfo,
            ...
            else
            {
                CAMX_LOG_DRQ("Failed to setup Node %s for Pipeline %u RequestId %llu Session %p",
                             m_ppNodes[nodeIndex]->NodeIdentifierString(), m_pipelineIndex, requestId, m_pSession);
            }
        }
            ...
            if (TRUE == Utils::IsBitSet(nodesEnabled, nodeIndex))
            {
                if (FALSE == GetFlushStatus())
                {
                    CAMX_LOG_DRQ("Queueing Node: %s on pipeline: %d for new requestId: %llu",
                                  m_ppNodes[nodeIndex]->NodeIdentifierString(), m_pipelineIndex, requestId);

                    result = m_pDeferredRequestQueue->AddDeferredNode(requestId, m_ppNodes[nodeIndex], NULL);
                ...
                else
                {
                    CAMX_LOG_DRQ("Skipping Node: %s on pipeline: %d for new requestId: %llu as Session %p in Flush state",
                                 m_ppNodes[nodeIndex]->NodeIdentifierString(), m_pipelineIndex, requestId, m_pSession);
                    m_ppNodes[nodeIndex]->Flush(requestId);
                }
            }
        }
    ...
    FinalizeInitializationData* pFinalizeInitializationData)
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        m_debugDataInitialized = (0 < HAL3MetadataUtil::DebugDataSize(DebugDataType::MaxTypes)) ? TRUE : FALSE;
    ...
    BOOL   earlyMetadata)
{
    ...
    else
    {
        ...
        if (CamxResultSuccess == result)
        {
            ...
            if (((TRUE  == IsRealTime()) && (0 != (dumpMetadata & 0x2))) ||
                ((FALSE == IsRealTime()) && (0 != (dumpMetadata & 0x1))))
            {
    ...
    UINT64 requestId)
{
    ...
    CAMX_ASSERT(NULL != pDebugData->pData);
    ...
    else
    {
        if ((FALSE == GetFlushStatus()) && (FALSE == m_pSession->IsResultHolderEmpty()))
        {
            ...
            if (m_invalidSOFCounter > (requestQueueDepth * 2))
            {
                ...
                // We only want to trigger recovery if there has been activity
                if ((FALSE == m_pSession->IsResultHolderEmpty()) && (m_lastCompletedRequestId != m_lastSubmittedRequestId) &&
                    (FALSE == delayRecovery))
                {
    ...
    switch (pMessage->type)
    {
        case CSLMessageTypeFrame:
            ...
            requestIdIndex = static_cast<UINT16>(pMessage->message.frameMessage.requestID %
                                                 (MaxPerRequestInfo * pPipeline->GetNumBatchedFrames()));
        ...
        case CSLMessageTypeError:
            ...
            for (UINT i = 0; i < pPipeline->m_nodeCount; i++)
            {
                pPipeline->m_ppNodes[i]->DumpDebugNodeInfo();
            }
    ...
    UINT32  indent)
{
    ...
    CAMX_LOG_TO_FILE(fd, indent, "name                          = \"%s\"", GetPipelineIdentifierString());
    CAMX_LOG_TO_FILE(fd, indent, "address                       = %p",     this);
    ...
    CAMX_LOG_TO_FILE(fd, indent, "currentRequest                = %llu",   m_lastSubmittedRequestId);
    CAMX_LOG_TO_FILE(fd, indent, "+------------------------------------------------------------------+");
    CAMX_LOG_TO_FILE(fd, indent, "Pending Nodes:");
    CAMX_LOG_TO_FILE(fd, indent, "Requests Range - lastInOrderCompletedRequestId: %llu currentRequest: %llu",
        ...
        requestId++)
    {
        ...
        CAMX_LOG_TO_FILE(fd, indent + 2, "RequestId = %llu, Nodes: %s", requestId, pendingNodeString);
    }
    ...
    for (UINT i = 0; i < MaxPerRequestInfo; i++)
    {
        ...
        {
            CAMX_LOG_TO_FILE(fd, indent, "RequestId = %llu", m_perRequestInfo[i].request.requestId);
            CAMX_LOG_TO_FILE(fd, indent + 2, "CSLSyncId                     = %u", m_perRequestInfo[i].request.CSLSyncID);
            CAMX_LOG_TO_FILE(fd, indent + 2, "numNodes                      = %u", m_nodeCount);
            CAMX_LOG_TO_FILE(fd, indent + 2, "numNodesRequestIdDone         = %u", m_perRequestInfo[i].numNodesRequestIdDone);
            CAMX_LOG_TO_FILE(fd, indent + 2, "numNodesPartialMetadataDone   = %u",
                             m_perRequestInfo[i].numNodesPartialMetadataDone);
            CAMX_LOG_TO_FILE(fd, indent + 2, "numNodesMetadataDone          = %u", m_perRequestInfo[i].numNodesMetadataDone);
            CAMX_LOG_TO_FILE(fd, indent + 2, "numNodesConfigDone            = %u", m_perRequestInfo[i].numNodesConfigDone);
            CAMX_LOG_TO_FILE(fd, indent + 2, "batchFrameIntervalNanoSeconds = %llu",
                             m_perRequestInfo[i].batchFrameIntervalNanoSeconds);
            CAMX_LOG_TO_FILE(fd, indent + 2, "numBatchedFrames              = %u",
    ...
    CAMX_LOG_TO_FILE(fd, indent, "Nodes + ports:");
    for (UINT i = 0; i < m_nodeCount; i++)
    {
...
VOID Pipeline::DumpDebugInfo()
{
        ...
        requestId++)
    {
        ...
        {
            CAMX_LOG_DUMP(CamxLogGroupCore, "Request Id: %lld", m_perRequestInfo[index].request.requestId);
            CAMX_LOG_DUMP(CamxLogGroupCore, "numNodes: %d", m_nodeCount);
            CAMX_LOG_DUMP(CamxLogGroupCore, "numNodesRequestIdDone: %d", m_perRequestInfo[index].numNodesRequestIdDone);
            CAMX_LOG_DUMP(CamxLogGroupCore, "numNodesPartialMetadataDone: %d",
                             m_perRequestInfo[index].numNodesPartialMetadataDone);
            CAMX_LOG_DUMP(CamxLogGroupCore, "numNodesMetadataDone: %d", m_perRequestInfo[index].numNodesMetadataDone);
            CAMX_LOG_DUMP(CamxLogGroupCore, "numNodesConfigDone: %d", m_perRequestInfo[index].numNodesConfigDone);
            ...
            {
                CAMX_LOG_ERROR(CamxLogGroupCore, "sequenceId[%d]: %d", j, m_perRequestInfo[index].pSequenceId[j]);
            }
        }
    }
...
UINT32 Pipeline::DetermineFrameDelay()
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        if (NULL != pFrameDelay)
        {
            ...
            CAMX_LOG_VERBOSE(CamxLogGroupCore, "Pipeline frame delay is %u for EIS", frameDelay);
        }
    }
...
{
    m_pInputPool->Flush(m_lastValidRequestIdbeforeFlush, m_lastCompletedRequestId);
    m_pMainPool->Flush(m_lastValidRequestIdbeforeFlush, m_lastCompletedRequestId);
    m_pInternalPool->Flush(m_lastValidRequestIdbeforeFlush, m_lastCompletedRequestId);
}
    ...
    UINT64 requestId)
{
    ...
    if (m_nodeCount == CamxAtomicIncU(&(pPerRequestInfo->numNodesRequestIdDone)))
    {
        ...
        if (TRUE == GetFlushStatus())
        {
            ...
            for (UINT64 reqId = (m_lastValidRequestIdbeforeFlush + 1);
        ...
        if (TRUE == CamxAtomicLoadU(&m_isRequestDoneSignalNeeded))
        {
...
VOID Pipeline::LogPendingNodes()
{
    ...
    CAMX_LOG_INFO(CamxLogGroupCore, "Session %p - Pipeline %s Pending Nodes\n"
                  "+------------------------------------------------------------------+\n"
                  "| Requests Range - lastInOrderCompletedRequestId: %llu currentRequest: %llu",
                  m_pSession,
                  GetPipelineIdentifierString(),
                  m_lastInOrderCompletedRequestId,
                  m_lastSubmittedRequestId);
...
VOID Pipeline::FlushPendingNodes()
{
        ...
        requestId++)
    {
        ...
        {
            PerRequestNodeStatus requestStatus = m_ppNodes[index]->GetRequestStatus(requestId);
            if (PerRequestNodeStatus::Setup == requestStatus || PerRequestNodeStatus::Running == requestStatus)
            {
