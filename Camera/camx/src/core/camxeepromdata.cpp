    ...
    CSLHandle               hCSL)
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        if (CamxResultSuccess != result)
        {
            CAMX_LOG_ERROR(CamxLogGroupSensor, "Failed to obtain CSL session");
        }
    }
    else
    {
    ...
    if (CamxResultSuccess == result)
    {
        ...
        {
            CAMX_LOG_ERROR(CamxLogGroupSensor, "Failed to read EEPROM data");
        }
    }
}
...
VOID EEPROMData::ParseMemoryMapData()
{
    ...
    CAMX_LOG_VERBOSE(CamxLogGroupSensor, "Number of memory blocks: %d", m_numberOfMemoryBlocks);
}
    ...
    UINT    memMapBlockIndex)
{
    ...
    result    = ImageSensorUtils::CreateRegisterSettingsCmd(pCmdBuffer, memMapSettingsCount,
                                                            pMemMapSettings, I2CFrequencyMode::FAST);
...
CamxResult EEPROMData::ReadEEPROMDevice()
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        createData.deviceIndices[0]             = m_EEPROMDeviceIndex;
    ...
    EndianType  endian)
{
    ...
    returnValue     = ((returnValue) << (BitsPerInteger - (numberOfBytes * (BitsPerByte)))) >> (BitsPerInteger - numberOfBits);
