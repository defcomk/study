    ...
    ChiContext*     pChiContext)
{
    ...
    {
        result = s_laserSensor.Initialize(pChiContext);
    ...
    ChiContext* pChiContext)
{
    ...
    else
    {
        if (NULL != pChiContext)
        {
            m_hTOFPollThHandle.pThreadManager = pChiContext->GetThreadManager();
            ...
            {
                CAMX_LOG_ERROR(CamxLogGroupNCS, "Failed to set up TOF link");
            }
        }
        else
        {
...
CamxResult TOFSensorIntf::SetupTOFLink()
{
    ...
    const CHAR*                   pTOFSensorLib   = NULL;
    const StaticSettings*         pStaticSettings = HwEnvironment::GetInstance()->GetStaticSettings();

    pTOFSensorLib = pStaticSettings->customTOFSensorLib;

    CAMX_LOG_VERBOSE(CamxLogGroupNCS, "Open TOF sensor library: %s", pTOFSensorLib);
    hSensorLib = OsUtils::LibMap(pTOFSensorLib);
    if (NULL == hSensorLib)
    {
