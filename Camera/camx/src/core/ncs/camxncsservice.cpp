    ...
    ChiContext* pChiContext)
{
    ...
    if (NULL != pChiContext)
    {
        ...
        for (UINT i = 0; i < MaxNCSIntfType; i++ )
        {
            ...
            if (i == QSEE)
            {
                pNCSQSEEObject = NCSIntfQSEE::GetInstance(pChiContext, this);
    ...
    else
    {
        ...
        CAMX_LOG_WARN(CamxLogGroupNCS, "Chi context pointer is null");
    }
    ...
    VOID* pPayload)
{
    ...
    if (NULL != pNCSThreadContext)
    {
        while (TRUE)
        {
            ...
            {
                CAMX_LOG_ERROR(CamxLogGroupNCS, "No incoming sensor data, Try again!");
                // Maybe do the reconfig the session here
            }
