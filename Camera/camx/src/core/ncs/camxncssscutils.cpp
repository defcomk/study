...
void SuidLookup::RequestSuid(std::string datatype)
{
    ...
    m_sscConnection.send_request(pb_req_msg_encoded);
}
    ...
    size_t          size)
{
    ...
    for (int i = 0; i < pb_event_msg.events_size(); i++)
    {
        ...
        {
            CAMX_LOG_ERROR(CamxLogGroupNCS, "invalid event msg_id=%d", pb_event.msg_id());
            continue;
        }
        ...
        CAMX_LOG_VERBOSE(CamxLogGroupNCS, "suid_event for %s, num_suids=%d, ts=%fs", datatype.c_str(),
