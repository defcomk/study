    ...
    NCSService*   pServiceObject)
{
    ...
    {
        result = intfObject.Initialize(pChiContext, pServiceObject);
...
CamxResult NCSIntfQSEE::QueryCapabilites()
{
    ...
    if (m_intfState == NCSIntfRunning)
    {
        for (INT sensorType = 0; sensorType < NCSMaxType; sensorType++)
        {
            ...
            {
                CAMX_LOG_WARN(CamxLogGroupNCS, "Sensor %d has not been successfully probed", sensorType);
            }
        }
    }
    m_pQSEEIntfMutex->Unlock();
            ...
            (QSEEConnRunning == m_sensorConnList[connIndex].connectionState))
        {
            ...
            if (NULL != pAsyncRequest)
            {
                ...
                if (NULL != pNode)
                {
                    ...
                    result = m_pChiContext->AttachChiFence(pFence);
    ...
    if (pClientRequestQ->NumNodes() > 0)
    {
        ...
        for (; NULL != pNode; pNode = pNext)
        {
            ...
            if (CamxResultSuccess == pJob->resultStatus)
            {
                ...
                m_pChiContext->ReleaseChiFence(hFence);
            }
    ...
    NCSSensorType  sensorType)
{
    ...
    if (m_intfState == NCSIntfRunning)
    {
        ...
        else
        {
            ...
            if (CamxResultSuccess == result)
            {
                CAMX_LOG_VERBOSE(CamxLogGroupNCS, "Probe success for sensor %d!", sensorType);
                // Request for its capabilities and fill the input pointer.
            ...
            {
                CAMX_LOG_ERROR(CamxLogGroupNCS, "Probe sensor %d with error %d!", sensorType, result);
                result = CamxResultEFailed;
            }
        }
    }
            ...
            switch (msg_id)
            {
                ...
                case SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_PHYSICAL_CONFIG_EVENT:
                {
                    ...
                    else
                    {
                        ...
                        m_sensorConnList[connIndex].curSamplingRate          = config.sample_rate();
                        ...
                        CAMX_LOG_INFO(CamxLogGroupNCS,
                            "Received config event for Conn Idx %d sensor type %d, with sample rate %f, tickPeriod %u",
                            connIndex,
                            sensorType,
                            config.sample_rate(),
                            m_sensorConnList[connIndex].bufferHandles.tickPeriod);
                ...
                case SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_EVENT:
                {
                    ...
                    {
                        CAMX_LOG_ERROR(CamxLogGroupNCS, "Unable to set fill data for sensor %d", sensorType);
                    }
                    break;
                }
    ...
    const sns_client_event_msg_sns_client_event &pb_event)
{
    ...
    if (m_sensorConnList[connIndex].bufferHandles.curPosLocked <= 0)
    {
        ...
        if (CamxResultSuccess == result)
        {
            ...
            newQSEEJob.timestamp = pb_event.timestamp();
    ...
    const sns_client_event_msg_sns_client_event &pb_event)
{
    ...
    for (UINT connIndex = 0; connIndex < NCSMaxSupportedConns; connIndex++)
    {
        if (m_sensorConnList[connIndex].calibSUID == suid)
        {
    ...
    NCSSensorDataHandle hAccesorObj)
{
    ...
    sensorType = pNCSSensorObj->GetSensorType();
    ClearBufferLock(sensorType);
