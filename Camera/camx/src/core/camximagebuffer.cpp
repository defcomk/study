    ...
    MemPoolBufMgrHandle             hMemPoolBufMgrHandle)
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        m_numBatchedFrames      = ((0 < pCreateData->numBatchedFrames) && (TRUE == pCreateData->allocateBufferMemory)) ?
        ...
        if (FALSE == CamX::IsChiNativeBufferType(&m_importData.bufferInfo))
        {
            ...
            else
            {
                CAMX_LOG_ERROR(CamxLogGroupMemMgr, "[%s][%p] Failed in importing Gralloc Buffer type=%d, BufferHandle=%p, "
                               ...
                               m_importData.flags, m_importData.deviceCount);
            }
        }
        else
        {
    ...
    BOOL isForced)
{
    ...
    {
        CAMX_ASSERT_ALWAYS_MESSAGE("[%s][%p] Warning: Release called on an un-backed buffer", GetBufferManagerName(), this);
    }
}
...
CamxResult ImageBuffer::BindImportedBuffer()
{
    ...
    if ((FALSE      == m_importData.valid)                  ||
        (ChiNative  != m_importData.bufferInfo.bufferType)  ||
