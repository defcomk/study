...
{
    CDKResult result = CDKResultEInvalidArg;
    CAMX_ASSERT((NULL != hMetaHandle) && (NULL != pPrivateData));
    ...
    CHIBufferManagerCreateData* pCreateData)
{
    ...
    if ((NULL != pBufferManagerName) && (NULL != pCreateData) && (NULL != pCreateData->pChiStream))
    {
        ...
        pImageFormat->format          = g_pChiContext->SelectFormat(pChiStream, { {0} });
        ...
        {
            UINT32 baseFDResolution = static_cast<UINT32>(g_pChiContext->GetStaticSettings()->FDBaseResolution);
    ...
    CHIHANDLE hSession)
{
    ...
    if (CDKResultSuccess == result)
    {
        result = pChiContext->FlushSession(pChiSession);
    }
