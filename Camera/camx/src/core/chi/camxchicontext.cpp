...
CamxResult ChiContext::Initialize()
{
    ...
    if (CamxResultSuccess == result)
    {
        m_pLock = Mutex::Create("CamxChiCtx");

        if (NULL == m_pLock)
        {
            CAMX_LOG_ERROR(CamxLogGroupHAL, "Failed to create mutex lock");
    ...
    if (CamxResultSuccess == result)
    {
        ...
        for (UINT key = 0; key < numCharacteristicsKeys; key++)
        {
            ...
            switch (pCameraInfo->pPlatformCaps->characteristicsKeys[key])
            {
                ...
                default:
                {
                    CAMX_ASSERT_ALWAYS_MESSAGE("Invalid static metadata key: %x",
    ...
    CHIPIPELINEINPUTOPTIONS*           pPipelineInputOptions)
{
    ...
    if (NULL != pPipelineDescriptor)
    {
        ...
        for (UINT streamId = 0; streamId < numOutputs; streamId++)
        {
            ...
            Format              selectedFormat      = SelectFormat(pChiStream, overrideImpDefinedFormat);
            ...
            {
                if (ChiExternalNode == pOutputBufferDescriptor[streamId].nodePort.nodeId)
                {

                    CAMX_LOG_INFO(CamxLogGroupChi, "StreamId=%d is associated with external Node %d and portID:%d",
                        streamId,
                        pOutputBufferDescriptor[streamId].nodePort.nodeId,
                        pOutputBufferDescriptor[streamId].nodePort.nodePortId);

                    SetChiStreamInfo(pChiStreamWrapper, numBatchedFrames, TRUE);
                }
                else
                {
                    SetChiStreamInfo(pChiStreamWrapper, numBatchedFrames, FALSE);
                }
                ...
                if (ChiStreamFormatImplDefined == pStream->format)
                {
                    ...
                    switch (pChiStreamWrapper->GetInternalFormat())
                    {
                        ...
                        case Format::YUV420NV12:
                            ...
                            if (GrallocUsagePrivateHEIF == (GrallocUsagePrivateHEIF & grallocUsage))
                            {
        ...
        if (pStream->pHalStream)
        {
            if (ChiStreamFormatImplDefined == pStream->format)
            {
                ...
                if (((overrideFormat == ChiStreamFormatUBWCTP10) ||
                        (overrideFormat == ChiStreamFormatUBWCNV12)) &&
                    (m_pHwEnvironment->GetPlatformStaticCaps()->ubwcVersion > UBWCver2))
                {
                    UBWCModeConfig1  UBWCModeConfig1 = { 0, UBWC_VERSION_3 };
                    ...
                    if (TRUE == IsUBWCLossySupported(pStream))
                    {
                        UBWCModeConfig1.lossyMode = 1;
                    ...
                    {
                        UBWCModeConfig1.lossyMode = 0;
                    }
                    ImageFormatUtils::SetUBWCModeConfig1(&UBWCModeConfig1);
                }
            }
    ...
    INT fd)
{
    ...
    while (NULL != pNode)
    {
        ...
        pSession->DumpState(fd, Indent + 2, SessionDumpFlag::ResetRecovery);
    ...
    const ChiStream*    pStream)
{
    ...
    if ((GrallocUsageHwVideoEncoder == (GrallocUsageHwVideoEncoder & (GetGrallocUsage(pStream))))   &&
    ...
    else if ((TRUE            == pStaticCaps->ubwcLossyPreviewSupported)     &&
