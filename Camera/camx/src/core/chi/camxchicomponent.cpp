    ...
    UINT16                 index)
{
    ...
    CamX::OSLIBRARYHANDLE   handle        = CamX::OsUtils::LibMap(pSoFilesName);
    ...
    UINT16                 index)
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupChi, "Failed to retrieve ChiAWBAlgorithmEntry function address. from lib:%s", pSoFilesName);
    }
    ...
    UINT*                  pNumExternalComponent)
{
    ...
    while (index < fileCountTypeNode + fileCountTypeStats + fileCountTypeHvx)
    {
        ...
        else if (index < (fileCountTypeNode+fileCountTypeStats))
        {
            ...
            {
                CAMX_LOG_VERBOSE(CamxLogGroupStats, "%s sofile %s token", soFilesName[index], outTokenString);
