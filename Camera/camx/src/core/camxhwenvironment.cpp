    ...
    if (CamxResultSuccess == result)
    {
        switch (CSLPlatform.family)
        {
            case CSLCameraFamilyTitan:
                // Titan 17X
                if ((1 == CSLPlatform.platformVersion.majorVersion) &&
            ...
            if (TRUE == bKeepImageSize)
            {
                ...
                for (UINT format = 0; format < numScalerFormats; format++)
                {
                        ...
                        (ScalerAvailableFormatsBlob                  == pPlatformCaps->scalerFormats[format]))
                    {
                        ...
                        /// @todo (CAMX-3585) Add minFrame Duration calculation logic at the right place
                        // 2. calculate the FPS throughput from BPS
                        maxBPSProcessingFPS = ((pPlatformCaps->maxBPSProcessingClockNom) / (width * height));

                        // 3. calculate the FPS throughput from IPE
                        if ((ScalerAvailableFormatsYCbCr420888           == pPlatformCaps->scalerFormats[format]) ||
                            (ScalerAvailableFormatsBlob                  == pPlatformCaps->scalerFormats[format]))
                        {
                            ...
                            maxIPEProcessingFPS = ((pPlatformCaps->maxIPEProcessingClockNom) /
                                                  ((width * height) + (1920 * 1080)));
                        }
                        else
                        {
                            // For video/preview (implementation defined format), consider IPE clock as SVS_L1
                            maxIPEProcessingFPS = ((pPlatformCaps->maxIPEProcessingClockSvsL1) / (width * height));
                        }

                        // 4. Supported FPS will be min of maxFPS, BPS FPS and IPE FPS.
                        supportedFPS = maxFPS;
                        ...
                        CAMX_LOG_VERBOSE(CamxLogGroupHWL, "supportedFPS = %d for width x height = %d x %d",
                                supportedFPS, width, height);
...
VOID HwEnvironment::InitializeJPEGMaxSizeStaticCaps()
{
    for (UINT sensorIndex = 0; sensorIndex < m_numberSensors; sensorIndex++)
    {
        ...
        {
            // Max JPEG buffer size is max JPEG image buffer size + JPEG header
            maxSizeInBytes = static_cast<INT32>((m_caps[sensorIndex].minStallDurations[i].width *
                m_caps[sensorIndex].minStallDurations[i].height * 3) + sizeof(Camera3JPEGBlob));
