    ...
    UINT               planeIndex)
{
    ...
    if (NULL != pTileInfo)
    {
        ...
        pCSLPlane->modeConfig   = ImageFormatUtils::GetUBWCModeConfig(pFormat, planeIndex);
        ...
        else
        {
            ...
            else
            {
                if (TRUE == ImageFormatUtils::IsRAW(pFormat))
                {
                    ...
                    pIOConfigs[pPacket->numBufferIOConfigs].planes[0].width =
                        ((pFormat->width * pFormat->formatParams.rawFormat.bitsPerPixel) + 7) / 8;
