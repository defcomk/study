...
DeferredRequestQueue::~DeferredRequestQueue()
{
    if (NULL != m_pThreadManager)
    {
        ...
        {
            m_pThreadManager->UnregisterJobFamily(DeferredWorkerWrapper, "DeferredWorkerWrapper", m_hDeferredWorker);
        }
    }
    ...
    DeferredRequestQueueCreateData* pCreateData)
{
    ...
    OsUtils::SNPrintF(&wrapperName[0], sizeof(wrapperName), "DeferredWorkerWrapper%d", s_numInstances++);
    ...
    if (CamxResultSuccess == result)
    {
        m_isPreemptDependencyEnabled = FALSE;
    ...
    Dependency* pDependency)
{
    ...
    if (NULL != pNode)
    {
        CAMX_LOG_DRQ("DRQ dispatching node=%d Node::%s, request=%llu, pipeline=%d, seqId=%d, bindIOBuffers=%d",
                     pNode->Type(),
        ...
        CAMX_LOG_DRQ("DRQ execute complete node=%d Node::%s, request=%llu, pipeline=%d, seqId=%d",
                     pNode->Type(),
                     pNode->NodeIdentifierString(),
                     pDependency->requestId,
                     pNode->GetPipelineId(),
    ...
    Dependency*      pDependency)
{
    ...
    if (NULL != pNode)
    {
        CAMX_LOG_DRQ("Adding dependencies for node: %d nodeName: %s:%d pipeline: %d request: %llu seqId: %d bindIOBuffers: %d",
                    pDependency->pNode->Type(),
                    pDependency->pNode->Name(),
                    pDependency->pNode->InstanceID(),
                    pDependency->pNode->GetPipelineId(),
                    pDependency->requestId,
                    pDependency->processSequenceId,
                    pDependency->bindIOBuffers);
        ...
        for (UINT i = 0; i < pDependency->propertyCount; i++)
        {
            ...
            INT64         offset  = (TRUE == pDependency->negate[i]) ? (-1 * pDependency->offsets[i])
            ...
            if (TRUE == m_logEnabled)
            {
                ...
                CAMX_LOG_DRQ("Node Dependency Name: %s:%d Pipeline: %d request: %llu seqId: %d -> "
    ...
    BOOL        isFlush)
{
    ...
    else
    {
        ...
        while (NULL != pNode)
        {
            ...
            if (reinterpret_cast<PipelineRequest*>(pNode->pData)->requestId + 20 < requestId)
            {
...
VOID DeferredRequestQueue::DispatchReadyNodes()
{
    ...
    if (TRUE == m_isPreemptDependencyEnabled)
    {
        m_pDeferredQueueLock->Lock();
        LightweightDoublyLinkedListNode* pDeferred = m_deferredNodes.Head();
        while (NULL != pDeferred)
        {
            ...
            if ((NULL != pDependency) && (TRUE == pDependency->preemptable))
            {
                CAMX_LOG_DRQ("Remove dependencies for node: %s:%d, request: %llu, pipeline: %d",
                                pDependency->pNode->Name(),
                                pDependency->pNode->InstanceID(),
                                pDependency->requestId,
                                pDependency->pNode->GetPipelineId());
    ...
    while (0 < m_readyNodes.NumNodes())
    {
        ...
        if (NULL != pReady)
        {
            ...
            if (NULL != pDependency)
            {
                ...
                UINT64 id = reinterpret_cast<UINT64>(pDependency);
                CAMX_TRACE_ASYNC_END_F(CamxLogGroupDRQ, id, "Deferred Node %s", pDependency->pNode->Name());
                ...
                if (CamxResultSuccess != result)
                {
                    CAMX_LOG_ERROR(CamxLogGroupCore, "Failed to post ProcessRequest job for node %s:%d, request %llu",
                                    pDependency->pNode->Name(), pDependency->pNode->InstanceID(), pDependency->requestId);
                }
                else
                {
                    CAMX_LOG_DRQ("Post job for node %s:%d, request %llu",
                                 pDependency->pNode->Name(),
                                 pDependency->pNode->InstanceID(),
                                 pDependency->requestId);
                }
    ...
    UINT64          requestId)
{
    ...
    for (UINT i = 0; i < pInput->propertyDependency.count; i++)
    {
        ...
        INT64         offset       = (TRUE == pSrcNegateList[i]) ? (-1 * pSrcOffsetList[i]) : pSrcOffsetList[i];
        INT64         intReq       = static_cast<INT64>(requestId);
        UINT64        request      = (intReq - offset <= 0) ? FirstValidRequestId : (intReq - offset);
        ...
        while (NULL != pDeferred)
        {
            ...
            if (NULL != pDependency)
            {
                if (pDependency->requestId == id)
                {
                    ...
                    CAMX_LOG_DUMP(CamxLogGroupDRQ, "+   Node: %s:%d, Request: %llu, SeqId: %d, Pipeline: %d,"
                    ...
                    for (UINT propertyIndex = 0; propertyIndex < pDependency->propertyCount; propertyIndex++)
                    {
                        ...
                        INT64         offset        = (TRUE == srcNegate) ? (-1 * srcOffset) : srcOffset;
    ...
    CSLFenceResult  result)
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupDRQ, "Fence failure, fence callback will not be called");
    }
    CAMX_FREE(pData);
    pData = NULL;
}
    ...
    while (NULL != pDeferred)
    {
        ...
        if (NULL != pDependency)
        {
            CAMX_LOG_TO_FILE(fd, indent + 2,
                             "Dependency %p - RequestId = %llu, Node::%s, Pipeline::%s_%u",
                             pDependency,
                             pDependency->requestId,
                             pDependency->pNode->NodeIdentifierString(),
                             pDependency->pNode->GetPipelineName(),
                             pDependency->pNode->GetPipelineId(),
                             pDependency->processSequenceId,
...
{
    m_isPreemptDependencyEnabled = isPreemptDependencyEnabled;
    CAMX_LOG_DRQ("Preempt Dependencies: %d", isPreemptDependencyEnabled);
}
                ...
                if ((NULL == pDependencyToRemove) || (pDependencyToRemove == pDependency))
                {
                    if (PropertyIDInvalid != pMapKey->dataId)
                    {
                        ...
                        if (TRUE == m_logEnabled)
                        {
                            ...
                            CAMX_LOG_DRQ("Property %08x %s published for node: %s:%d pipeline: %d request: %llu "
                    ...
                    {
                        pDependency->signaledCount++;
                        CAMX_LOG_DRQ("Fence %08x(%08x) signaled for node: %s:%d pipeline: %d request: %llu "
                                     "seqId: %d bindIOBuffers: %d",
                                     pMapKey->pFence,
    ...
    Dependency* pDependency)
{
    ...
    for (UINT i = 0; i < pDependency->propertyCount; i++)
    {
        INT64         offset   = (TRUE == pDependency->negate[i]) ? (-1 * pDependency->offsets[i]) : pDependency->offsets[i];
        INT64         intReq   = static_cast<INT64>(pDependency->requestId);
        UINT64        request  = (intReq - offset <= 0) ? FirstValidRequestId : (intReq - offset);
        DependencyKey mapKey   = { request, pDependency->pipelineIds[i], pDependency->properties[i], NULL, NULL };
