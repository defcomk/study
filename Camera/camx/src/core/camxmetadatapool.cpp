    ...
    UINT            numPrePublishedTags)
{
    ...
    pLocalInstance = CAMX_NEW MetadataPool(poolType, pipelineId, pThreadManager, numSlots, pPipelineName, numPrePublishedTags);
    ...
    UINT64 flushRequestId)
{
    ...
    if (PoolType::PerFrameResult == m_poolType)
    {
        ...
        if (NULL != pMetaBuffer)
        {
            m_pStickyMetaBuffer->Copy(pMetaBuffer, FALSE);
        }
    ...
    MetaBuffer** ppMetabuffer)
{
    ...
    CAMX_ASSERT(NULL != m_pMetaBuffer);
    CAMX_ASSERT(m_pPool->UseMetaBuffers());
    ...
    MetaBuffer** ppMetabuffer)
{
    ...
    {
        *ppMetabuffer = NULL;
        CAMX_LOG_WARN(CamxLogGroupMeta, "Metabuffer already detached");
    ...
    const CHAR* pName)
{
    ...
    {
        CAMX_LOG_VERBOSE(CamxLogGroupMeta,
                         "Invalid Slot to get a metadata from pool %d tag %x reqId %llu name %s client %s",
