    ...
    MemPoolBufferManager* pMemPoolBufMgr)
{
    ...
    if (CamxResultSuccess == result)
    {
        PrintMemPoolBufMgrState(pMemPoolBufMgr);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
MemPoolBufferHandle MemPoolGroup::GetBufferFromPool(
    MemPoolBufferManager*   pMemPoolBufMgr,
    ...
    BufferHandle*           phGrallocBuffer)
{
    ...
    return static_cast<MemPoolBufferHandle>(pBuffer);
}
    ...
    if (0 < numBuffersToFree)
    {
        while (numBuffersFreed != numBuffersToFree)
        {
            ...
            else
            {
                ...
                PrintMemPoolBufferState(pBuffer);
...
{
    if (TRUE == GetStaticSettings()->enableMPMStatelogging)
    {
        CAMX_LOG_VERBOSE(CamxLogGroupMemMgr, "MemPoolGroup[%s]-->MemPoolBufMgr[%s] : Printing state",
                         m_pGroupName, pMemPoolBufMgr->name);

        CAMX_LOG_VERBOSE(CamxLogGroupMemMgr, "MemPoolGroup[%s]-->MemPoolBufMgr[%s] : Activated=%d, InUse=%d, PeakUsed=%d",
                         m_pGroupName, pMemPoolBufMgr->name, pMemPoolBufMgr->bActivated,
                         pMemPoolBufMgr->memPoolBufferList.NumNodes(), pMemPoolBufMgr->peakBuffersUsed);
        ...
        while (NULL != pNode)
        {
            ...
            PrintMemPoolBufferState(pMemPoolBuffer);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID MemPoolGroup::PrintMemPoolGroupState() const
{
    if (TRUE == GetStaticSettings()->enableMPMStatelogging)
    {
        CAMX_LOG_VERBOSE(CamxLogGroupMemMgr, "MemPoolGroup[%s][%p] : Printing state------------------------------",
                         m_pGroupName, this);
        ...
        CAMX_LOG_VERBOSE(CamxLogGroupMemMgr, "MemPoolGroup[%s][%p] : --------------Buffer Managers : Registered=%d, Active=%d",
        ...
        CAMX_LOG_VERBOSE(CamxLogGroupMemMgr, "MemPoolGroup[%s][%p] : Printing state Done---------------------",
                         m_pGroupName, this);
    }
}
...
VOID MemPoolGroup::PrintMemoryPoolManagerStats()
{
    if (TRUE == GetStaticSettings()->enableMemoryStats)
    {
        ...
        CAMX_LOG_INFO(CamxLogGroupMemMgr, "MemPoolMgrStats : MapsCPU : num=%d, peakNum=%d, size=%zu, peakSize=%zu; "
    ...
    ) const
{
    ...
    // add settings
    // If the size is more than 15MB, have diff range more 1MB
    if (groupSize >= 15000000)
    {
        sizeDiffAllowed = 1000000;
    }
    ...
    MemPoolBufferManager*   pMemPoolBufMgr)
{
    if (TRUE == GetStaticSettings()->MPMSelfTuneImmediateCount)
    {
        ...
        if (BufferManagerType::CamxBufferManager == pMemPoolBufMgr->createData.bufferManagerType)
        {
            ...
            {
                pMemPoolBufMgr->createData.immediateAllocBufferCount = 4;
            }
            else if (TRUE == pMemPoolBufMgr->createData.linkProperties.isPartOfRealTimePipeline)
            {
                pMemPoolBufMgr->createData.immediateAllocBufferCount = 2;
            }
            else
            {
