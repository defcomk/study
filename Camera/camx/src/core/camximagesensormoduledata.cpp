...
CamxResult ImageSensorModuleData::Probe(
    ...
    INT32*  pDeviceIndex)
{
    /// @todo (CAMX-555): use CSL packet/buffer functions after they are available
    ...
    /// @todo (CAMX-555): populate alignment properly
    ...
    const HwDeviceTypeInfo* pCSLDeviceTable)
{
    ...
    if (CamxResultSuccess == GetCameraId(&cameraId))
    {
        ...
        if (NULL != pActuatorDriverData)
        {
            ...
            {
                CAMX_LOG_INFO(CamxLogGroupSensor, "Actuator device found on camera %d", cameraId);
    ...
    UINT                    cameraID)
{
    ...
    if ((NULL != pCapability) && (NULL != pTuningManager))
    {
        ...
        if (NULL != pLensInfo)
        {
            ...
            {
                CAMX_LOG_ERROR(CamxLogGroupSensor, "focalLengths[0] is 0");
            }
        }
        else
        {
...
{
    const PDAFConfigurationData* pPDAFConfigData = GetPDAFConfigDataObj();

    CAMX_ASSERT(NULL != pIsSupported);
    CAMX_ASSERT(NULL != pPDType);

    *pIsSupported = FALSE;
    ///@ todo (CAMX-3997) Add is pdaf supported flag to sensor driver for each resolution.

    if (modeIndex < m_pSensorData->GetResolutionInfo()->resolutionDataCount)
    {
        if (NULL != pPDAFConfigData)
        {
            *pPDType = pPDAFConfigData->PDInfo.PDType;

            if ((PDAFType::PDType3 == *pPDType) && (NULL != pPDAFConfigData) &&
                (modeIndex < pPDAFConfigData->PDSensorNativePatternInfoCount))
            {
                // For PDAF Type3 there is no stream information in the driver. CAMIF calculates the data pattern
                // based on the native pattern supplied.
                // If the binary for this sensor contains Type3 PDAF information, then return TRUE
                *pIsSupported = TRUE;
            }
            else
            {
                ...
                {
                    if (StreamType::PDAF == resolutionData.streamInfo.streamConfiguration[index].type)
                    {
                        *pIsSupported = TRUE;
                        break;
                    }
...
VOID ImageSensorModuleData::Get3ExposureHDRInformation(
    UINT32                   modeIndex,
    BOOL*                    pIsSupported,
    HDR3ExposureInformation* pHDR3ExposureType
    ) const
{
    CAMX_ASSERT(NULL != pIsSupported);
    CAMX_ASSERT(NULL != pHDR3ExposureType);

    HDR3ExposureInformation HDR3ExposureType = m_pSensorData->Get3ExposureHDRInfo(modeIndex);
    ...
    *pIsSupported = FALSE;
    ...
    if (modeIndex < m_pSensorData->GetResolutionInfo()->resolutionDataCount)
    {
        ...
        for (UINT index = 0; index < resolutionData.streamInfo.streamConfigurationCount; index++)
        {
            ...
            {
                if (HDR3ExposureInformation::HDR3ExposureType1 == HDR3ExposureType ||
                    HDR3ExposureInformation::HDR3ExposureType2 == HDR3ExposureType ||
                    HDR3ExposureInformation::HDR3ExposureType3 == HDR3ExposureType)
                {
                    *pIsSupported = TRUE;
                    switch(HDR3ExposureType)
                    {
                        case HDR3ExposureInformation::HDR3ExposureType1:
                            *pHDR3ExposureType = HDR3ExposureInformation::HDR3ExposureType1;
                            break;
                        case HDR3ExposureInformation::HDR3ExposureType2:
                            *pHDR3ExposureType = HDR3ExposureInformation::HDR3ExposureType2;
                            break;
                        case HDR3ExposureInformation::HDR3ExposureType3:
                            *pHDR3ExposureType = HDR3ExposureInformation::HDR3ExposureType3;
                            break;
                        default:
                            *pHDR3ExposureType = HDR3ExposureInformation::HDR3ExposureTypeUnknown;
                            break;
                    }
                    break;
                }
