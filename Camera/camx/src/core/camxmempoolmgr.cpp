    ...
    const BufferManagerCreateData*  pCreateData)
{
    ...
    if ((CamxResultSuccess == result) &&
        (FALSE             == MemPoolMgr::IsSupportedBufferHeap(pCreateData->bufferProperties.bufferHeap)))
    {
    ...
    BufferHandle*       phGrallocBuffer)
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        hBufferHandle = pMemPoolGroup->GetBufferFromPool(pBufMgrHandleData->pMemPoolBufMgr, pCSLBufferInfo, phGrallocBuffer);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID MemPoolMgr::PrintMemPoolMgrState() const
{
    if (TRUE == GetStaticSettings()->enableMPMStatelogging)
    {
        ...
        if (NULL != pMemPoolMgr)
        {
            CAMX_LOG_VERBOSE(CamxLogGroupMemMgr, "MemPoolMgr[%p] : Printing State============================================",
                             pMemPoolMgr);

            CAMX_LOG_VERBOSE(CamxLogGroupMemMgr, "s_isValid=%d, m_groupNameCounter=%d, Number of Groups=%d",
            ...
            while (NULL != pNode)
            {
                ...
                pMemPoolGroup->PrintMemPoolGroupState();

                pNode = LightweightDoublyLinkedList::NextNode(pNode);
            }

            CAMX_LOG_VERBOSE(CamxLogGroupMemMgr, "MemPoolMgr[%p] : Printing State Done=======================================",
                             pMemPoolMgr);
    ...
    , m_pMonitorThreadCond(NULL)
{
    ...
    if ((NULL == m_pLock) || (NULL == m_pMonitorThreadCond))
    {
