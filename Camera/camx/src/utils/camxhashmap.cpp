    ...
    VOID* pVal)
{
    ...
    {
        LDLLNode*    pNewLLNode = static_cast<LDLLNode*>(CAMX_CALLOC(sizeof(LDLLNode)));
        HashmapNode* pNode      = static_cast<HashmapNode*>(CAMX_CALLOC(m_realHashmapNodeSize));
    ...
    if (NULL != m_ppTable)
    {
        for (UINT i = 0; i < m_params.maxNumBuckets; i++)
        {
            if (NULL != m_ppTable[i])
            {
                ...
                CAMX_DELETE(m_ppTable[i]);
                m_ppTable[i] = NULL;
            }
        }
        CAMX_DELETE [] m_ppTable;
        m_ppTable = NULL;
    }
}
    ...
    const HashmapParams* pParams)
{
    ...
    m_ppTable = CAMX_NEW LightweightDoublyLinkedList*[m_params.maxNumBuckets];
    if (NULL == m_ppTable)
    {
    ...
    else
    {
        ...
        if (TRUE == m_params.preallocateBuckets)
        {
            for (UINT i = 0; i < m_params.maxNumBuckets; i++)
            {
                m_ppTable[i] = CAMX_NEW LightweightDoublyLinkedList();
            }
        }
    }
    ...
    ) const
{
    ...
    {
        m_ppTable[index] = CAMX_NEW LightweightDoublyLinkedList();
    }
